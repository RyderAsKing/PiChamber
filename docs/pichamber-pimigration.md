...
Approved design plan. This document records the agreed migration from the current OpenCode-backed product to a Pi-native PiChamber. The completed boundary, daemon lifecycle, session UI, and provider/settings work are recorded in their implementation records: [Workstream 0](./pichamber-pi-workstream-0.md), [daemon lifecycle](./pichamber-pi-workstream-1.md), [Pi-native session UI](./pichamber-pi-workstream-3.md), [providers/settings](./pichamber-pi-workstream-4.md), and [native resources/trust](./pichamber-pi-workstream-5.md). Workstreams 1–5 have focused exit-gate validation; the final release and OpenCode-runtime removal gates remain outstanding.
