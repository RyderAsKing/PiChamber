...
- Direct runtime SDK clients outside the shared facade currently include `packages/web/server/lib/openchamber-sessions/routes.js`, `packages/web/server/lib/openchamber-control/service.js`, and `packages/web/server/lib/opencode/skill-routes.js`.
...
- `packages/web/server/lib/system-prompt/` and its managed-launch injection path, plus `packages/web/server/lib/session-assist/`, scheduled tasks, loop-file scheduling, and their UI/settings/CLI consumers have been removed as OpenCode-coupled features. `packages/web/server/lib/agent-tool/` and its callers remain OpenCode-coupled feature removal targets; they are not Pi daemon homes.
...
| `/api/openchamber/events` | `packages/web/server/lib/openchamber-sessions/events.js` | Temporary low-frequency legacy session-control event stream. Remove it with the remaining OpenCode session/control adapters; Pi session events use `/api/pi/events`. |
