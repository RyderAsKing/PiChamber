...
Plus everything from the shared PiChamber UI: branchable timeline, smart tool UIs, voice mode, Git workflows, and more.

## Commands

| Command | Description |
|---------|-------------|
| `PiChamber: Focus Chat` | Focus the chat panel |
| `PiChamber: New Session` | Start a new chat session |
| `PiChamber: Open Sidebar` | Open the PiChamber sidebar |
| `PiChamber: Open Agent Manager` | Launch parallel multi-model runs |
| `PiChamber: Open Session in Editor` | Open current or new session in an editor tab |
| `PiChamber: Settings` | Open extension settings |
| `PiChamber: Restart API Connection` | Restart the OpenCode API process |
| `PiChamber: Show OpenCode Status` | Debug info for development or bug reports |

### Right-click menu

Select code in the editor, right-click, and find the **PiChamber** submenu:
...
|---------|---------|-------------|
| `pichamber.apiUrl` | _(empty)_ | URL of an external OpenCode API server. Leave empty to auto-start a local instance. |
| `pichamber.opencodeBinary` | _(empty)_ | Absolute path to the `opencode` CLI binary. Useful when PATH lookup fails. Requires window reload to apply. |

> Setting keys were renamed from `openchamber.*` to `pichamber.*` in this release. Existing user settings will need to be updated manually.
...
- `PICHAMBER_VSCODE_BIN=cursor bun run vscode:dev`
- `PICHAMBER_VSCODE_DEV_WORKSPACE=/path/to/workspace bun run vscode:dev`
...
Install locally: `code --install-extension packages/vscode/pichamber-*.vsix`

> The `OPENCHAMBER_VSCODE_BIN` and `OPENCHAMBER_VSCODE_DEV_WORKSPACE` environment variables are runtime contracts and are deferred to a later phase.
