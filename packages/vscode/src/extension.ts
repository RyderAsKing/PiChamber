...
const SETTINGS_KEY = 'pichamber.settings';
...
export async function activate(context: vscode.ExtensionContext) {
  outputChannel = vscode.window.createOutputChannel('PiChamber');
  ...
  const attemptMoveChatToRightSidebar = async (): Promise<'moved' | 'unsupported' | 'failed'> => {
    ...
    try {
      await vscode.commands.executeCommand('pichamber.chatView.focus');
      await vscode.commands.executeCommand(moveCommandId);
      return 'moved';
    } catch (error) {
      outputChannel?.appendLine(
        `[PiChamber] Failed moving chat view to right sidebar (command=${moveCommandId}): ${error instanceof Error ? error.message : String(error)}`
      );
      return 'failed';
    }
  };
  ...
  const maybeMoveChatToRightSidebarOnStartup = async () => {
    ...
    const attempted = context.globalState.get<boolean>('pichamber.sidebarAutoMoveAttempted') || false;
    if (attempted) return;
    await context.globalState.update('pichamber.sidebarAutoMoveAttempted', true);
  ...
  const config = vscode.workspace.getConfiguration('pichamber');
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.openSidebar', async () => {
      // Best-effort: open the container (if available), then focus the chat view.
      try {
        await vscode.commands.executeCommand('workbench.view.extension.pichamber');
      } catch (e) {
        outputChannel?.appendLine(`[PiChamber] workbench.view.extension.pichamber failed: ${e}`);
      }

      try {
        await vscode.commands.executeCommand('pichamber.chatView.focus');
      } catch (e) {
        outputChannel?.appendLine(`[PiChamber] pichamber.chatView.focus failed: ${e}`);
        vscode.window.showErrorMessage(t('PiChamber: Failed to open sidebar - {0}', String(e)));
        return false;
      }

      if (!chatViewProvider?.hasResolvedView()) {
        outputChannel?.appendLine('[PiChamber] Chat sidebar focus completed before the webview was resolved');
        vscode.window.showWarningMessage(t('PiChamber: Chat sidebar is not ready'));
        return false;
      }
  ...
  const revealChatViewForPayload = async () => {
    const opened = await vscode.commands.executeCommand<boolean>('pichamber.openSidebar');
    if (!opened) {
      return false;
    }
    ...
    if (!chatViewProvider?.hasResolvedView()) {
      outputChannel?.appendLine('[PiChamber] Chat sidebar webview was disposed before payload delivery');
      vscode.window.showWarningMessage(t('PiChamber: Chat sidebar is not ready'));
      return false;
    }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.focusChat', async () => {
      await vscode.commands.executeCommand('pichamber.chatView.focus');
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.internal.settingsSynced', (settings: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.internal.permissionAutoAcceptSynced', (snapshot: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.openAgentManager', () => {
      agentManagerProvider?.createOrShow();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.setActiveSession', (sessionId: unknown, title?: unknown) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.openActiveSessionInEditor', () => {
      if (!activeSessionId) {
        vscode.window.showInformationMessage(t('PiChamber: No active session'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.openSessionInEditor', (sessionId: string, title?: string) => {
      if (typeof sessionId !== 'string' || sessionId.trim().length === 0) {
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.openNewSessionInEditor', () => {
      sessionEditorProvider?.createOrShowNewSession();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.openCurrentOrNewSessionInEditor', () => {
      if (activeSessionId) {
        sessionEditorProvider?.createOrShow(activeSessionId, activeSessionTitle ?? undefined);
      } else {
        sessionEditorProvider?.createOrShowNewSession();
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.restartApi', async () => {
      try {
        ...
        vscode.window.showInformationMessage(t('PiChamber: API connection restarted'));
      } catch (e) {
        vscode.window.showErrorMessage(t('PiChamber: Failed to restart API - {0}', String(e)));
      }
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.addToContext', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('PiChamber [Add to Context]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('PiChamber [Add to Context]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.attachExplorerToChat', async (resource?: vscode.Uri, resources?: vscode.Uri[]) => {
      ...
      if (attachedFiles.length === 0) {
        vscode.window.showWarningMessage(t('PiChamber: No file selected to mention'));
        return;
      }
      ...
      if (skippedEntries.length > 0) {
        vscode.window.showInformationMessage(t('PiChamber: Some selected entries were skipped (folders or unsupported resources)'));
      }
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.explain', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('PiChamber [Explain]: No active editor'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.improveCode', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('PiChamber [Improve Code]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('PiChamber [Improve Code]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.newSession', async (directory?: unknown) => {
      ...
      if (!folderPath && candidates.length === 0) {
        vscode.window.showInformationMessage('PiChamber: No folder is open. Open a folder to start a new session.');
        return;
      }
      ...
      if (openCodeManager) {
        ...
        if (!result.success) {
          vscode.window.showErrorMessage(`PiChamber: ${result.error}`);
          return;
        }
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.showSettings', () => {
      chatViewProvider?.showSettings();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('pichamber.showOpenCodeStatus', async () => {
      const config = vscode.workspace.getConfiguration('pichamber');
      ...
      const lines = [
        ...
        `PiChamber version: ${extensionVersion || '(unknown)'}`,
        ...
        `OpenCode binary (configured): ${(vscode.workspace.getConfiguration('pichamber').get<string>('opencodeBinary') || '').trim() || '(none)'}`,
