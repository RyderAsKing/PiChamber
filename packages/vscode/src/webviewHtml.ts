...
export function getWebviewHtml(options: WebviewHtmlOptions): string {
  ...
  // 
  // Logo geometry matches PiChamberLogo.tsx:
...
<head>
  ...
  <title>PiChamber</title>
</head>
<body>
  <!-- Initial loading screen with simplified PiChamber logo -->
  ...
  <script>
    ...
    function getBootstrapMessages() {
      var locale = 'en';
      try {
        var rawLocale = window.localStorage.getItem('pichamber.i18n.v1');
        if (rawLocale) {
  ...
  <script type="module">
    ...
    if (!devServerUrl) {
      ...
      const getDevMessages = () => {
        try {
          const rawLocale = window.localStorage.getItem('pichamber.i18n.v1');
          if (rawLocale) {
      ...
      const tryLoadDevBundle = () => {
        ...
        Promise.resolve()
          ...
          .catch((error) => {
            attempt += 1;
            console.warn('[PiChamber] VS Code webview dev bundle unavailable, retrying...', error);
