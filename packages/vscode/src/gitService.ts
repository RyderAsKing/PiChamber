...
const formatWorktreePopulateError = (message: string | null | undefined): string => {
  ...
  return [
    ...
    'PiChamber enables Git `core.longpaths` for worktree population; if this still fails on Windows, enable OS long paths (LongPathsEnabled) or open the repository from a shorter absolute path.',
  ].join('\n');
};
...
const ensureWorktreeLongpaths = async (directory: string): Promise<void> => {
  ...
  // subsequent PiChamber and CLI git operations in this repo also get long
...
// sandbox itself when an instance boots for that directory. PiChamber used to
...
export async function applyGitHunk(
  ...
  const tmpPath = path.join(tmpDir, `pichamber-hunk-${Date.now()}-${Math.random().toString(36).slice(2)}.patch`);
...
export async function stashGitChanges(directory: string, options: { message?: string } = {}): Promise<{ success: boolean; created: boolean; message: string; output: string }> {
  const message = options.message?.trim() || `PiChamber stash ${new Date().toISOString()}`;
