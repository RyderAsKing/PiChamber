...
description: Connect the desktop app to PiChamber on another machine over SSH.
---
...
PiChamber walks through the steps — checking the connection, setting up the remote, starting the server, and forwarding the port — and shows where it is at each stage. When it reaches **ready**, the remote UI loads locally.

## Credentials

You decide whether to save the SSH and UI passwords or enter them each time. If the connection drops, PiChamber reports which step failed so you can fix it — see [Remote access](/troubleshooting/remote-access/).

## Connection links

If a remote machine already runs PiChamber, the easiest way to connect the desktop app is a pairing link. On the remote server's UI, open **Settings → Remote Instances → Connect to this server → Add a device**, create a link, and import it on your desktop at **Settings → Remote Instances → Other PiChamber servers → Import Link**. See [Connect a Device](/connect-devices/) for the full flow.
