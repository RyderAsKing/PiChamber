---
title: トンネル
description: リモートおよびモバイルアクセス向けに PiChamber を安全に公開します。
---

# トンネル

トンネルは PiChamber への公開リンクです。別ネットワークの普通のブラウザからアクセスできます。実行中のインスタンスに対して作成するには `pichamber tunnel` を使います。
...
PiChamber はあなたのマシン上でトンネルプロバイダー CLI を起動します。使いたいプロバイダーを先にインストールしてください。
...
この手順を省いた場合、`pichamber tunnel start` は CLI サーバーを自動起動できます。自動起動時には `--port`、`--host`、`--lan`、`--ui-password`、`--api-only` などのサーバーオプションを渡せます。
...
トンネルが起動すると、`status` に公開 URL が表示されます。それを開くか QR コードをスキャンすると、どこからでも PiChamber にアクセスできます。

デフォルトでは、PiChamber は対話型 TTY セッションで QR コードを表示します。QR 出力を強制するには `--qr`、無効にするには `--no-qr` を使います。
...
- PiChamber インスタンス（ポート）ごとにアクティブなトンネルは 1 つです
...
- [モバイルアプリと PWA](/mobile/) — スマートフォンから PiChamber にアクセスする
