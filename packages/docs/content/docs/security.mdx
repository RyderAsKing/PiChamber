...
PiChamber gives access to your machine and your code, so lock it down before anyone but you can reach it. This page covers the UI password, passkeys, and what to know before exposing PiChamber to a network.

## Set a UI password

Start PiChamber with a password and the browser UI asks for it:

```bash
pichamber --ui-password be-creative-here
```

You can also set it with the `OPENCHAMBER_UI_PASSWORD` environment variable instead of putting it on the command line. After signing in, PiChamber remembers the device for a while so you're not asked every time.
...
Once a password is set, you can add passkeys (Face ID, Touch ID, a security key) for quicker sign-in. Add them at **Settings → PiChamber → Passkeys**.
...
- By default PiChamber only listens on your own machine (`127.0.0.1`). It takes a deliberate change to listen more widely, and you should set a password first.
...
- If you put PiChamber behind your own HTTPS server, see [Reverse Proxy](/reverse-proxy/).
...
- [Reverse Proxy](/reverse-proxy/) — run PiChamber behind your own server
