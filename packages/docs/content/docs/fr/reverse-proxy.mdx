---
title: Reverse proxy
description: Configurez correctement PiChamber derrière Nginx, Nginx Proxy Manager ou un autre reverse proxy.
---

# Reverse proxy

Utilisez cette page si vous exécutez PiChamber derrière Nginx, Nginx Proxy Manager, Caddy, Cloudflare ou un autre reverse proxy.

## Avant de le proxifier

1. Confirmez d’abord qu’PiChamber fonctionne en accès direct.
...
- Désactivez gzip sur le proxy si PiChamber compresse déjà les réponses.
...
- PiChamber joignable directement sur le LAN
...
- PiChamber compresse les réponses HTTP avec gzip (seuil de 1 Ko).
...
Les routes de streaming SSE sont exclues de la compression par PiChamber, mais le CDN peut encore les bufferiser. Consultez la documentation de votre CDN pour désactiver le buffering sur les chemins SSE.
