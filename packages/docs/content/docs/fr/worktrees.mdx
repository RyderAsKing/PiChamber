...
3. Confirmez le dossier du worktree (PiChamber en propose un à partir du nom de branche).
4. Créez-le.

PiChamber crée la branche, prépare le dossier et démarre une session dedans. Vous pouvez aussi en lancer une directement depuis une [todo](/notes-todos-plans/) ou une [issue ou PR GitHub](/github/).
...
Un worktree peut demander votre attention si son dossier a disparu, si sa branche est détachée, ou si un merge ou rebase est à moitié terminé. PiChamber signale ces cas pour que vous puissiez les corriger — voir [Worktrees et Git](/troubleshooting/worktrees-git/).

## Pages liées

- [Multi-run](/multi-run/) — lancer plusieurs sessions worktree à la fois
- [Workflows Git et GitHub](/git/) — commiter et intégrer depuis PiChamber
