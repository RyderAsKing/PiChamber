---
title: Túneles
description: Expón PiChamber de forma segura para acceso remoto y móvil.
---

# Túneles

Un túnel es un enlace público a tu PiChamber, para que puedas acceder a él desde un navegador corriente en otra red. Usa `pichamber tunnel` para crear uno para una instancia en marcha.
...
PiChamber inicia el CLI del proveedor de túnel en tu máquina. Instala primero el proveedor que quieras usar:
...
Si omites este paso, `pichamber tunnel start` puede iniciar automáticamente un servidor CLI. Al hacerlo, puedes pasar opciones del servidor como `--port`, `--host`, `--lan`, `--ui-password` y `--api-only`.
...
Cuando el túnel está activo, `status` muestra una URL pública. Ábrela, o escanea el código QR, para acceder a PiChamber desde cualquier lugar.

Por defecto, PiChamber imprime un código QR en sesiones TTY interactivas. Usa `--qr` para forzar la salida del QR, o `--no-qr` para desactivarla.
...
- un único túnel activo por instancia de PiChamber (puerto)
...
- [PWA y acceso móvil](/es/mobile/) — accede a PiChamber desde tu teléfono
