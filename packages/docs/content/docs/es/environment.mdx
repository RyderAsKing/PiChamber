...
description: Configura la integración de PiChamber y OpenCode con variables de entorno.
---

# Variables de entorno

PiChamber lee estas variables al arrancar. Para servicios de inicio, `pichamber startup enable` guarda por defecto una copia del entorno actual, así que vuelve a ejecutarlo después de cambiar variables que quieras usar en el servicio.

## Servidor PiChamber

### `OPENCHAMBER_HOST`

Dirección donde escucha el servidor web de PiChamber. Usa `0.0.0.0` para permitir acceso desde otras máquinas.
...
Inicia PiChamber en modo headless cuando vale `true` o `1`. Las rutas API siguen disponibles para clientes de escritorio y móviles, pero no se sirve la UI del navegador.

### `OPENCHAMBER_DATA_DIR`

Cambia el directorio de datos de PiChamber. Por defecto es `~/.config/pichamber`.
...
Conecta PiChamber a un servidor OpenCode existente. Debe ser un origin `http` o `https` con puerto explícito y sin ruta, query ni hash. `OPENCODE_HOST` tiene prioridad sobre `OPENCODE_PORT`.
...
Evita que PiChamber arranque su propio servidor OpenCode cuando vale `true`.

### `OPENCHAMBER_OPENCODE_HOSTNAME`

Hostname para el servidor OpenCode gestionado por PiChamber. Por defecto es `127.0.0.1`.

### `OPENCODE_BINARY`

Ruta al ejecutable `opencode` que debe usar PiChamber.
...
Alias específico de PiChamber para elegir la distro WSL. `OPENCODE_WSL_DISTRO` tiene prioridad si ambas están definidas.
...
Ejecutable de shell para las sesiones de terminal en PiChamber.

### `OPENCHAMBER_GIT_BINARY`

Ejecutable de Git usado por las funciones Git de PiChamber.

### `GIT_BINARY`

Override alternativo para el ejecutable de Git. Para ajustes específicos de PiChamber, prefiere `OPENCHAMBER_GIT_BINARY`.
...
API key para funciones de voz de PiChamber que llaman a servicios compatibles con OpenAI.
...
Token de autenticación de ngrok usado por los comandos de túnel de PiChamber. También puedes configurarlo con `ngrok config add-authtoken <token>`.

## Ayudas de runtime

### `BUN_BINARY`

Ejecutable de Bun que PiChamber debe usar al iniciar procesos daemon.

### `BUN_INSTALL`

Raíz de instalación de Bun. PiChamber la usa para encontrar `bin/bun` al iniciar daemons y actualizar.
