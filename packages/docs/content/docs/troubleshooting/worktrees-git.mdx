...
PiChamber flags a worktree when something's off:

- **folder missing** — the worktree's folder was deleted or moved outside PiChamber; remove the session and create a fresh worktree
...
- on Windows, git uses Unix-style paths (like `/c/Users/...`) — PiChamber handles this, but custom SSH key paths should follow the same form
