---
title: Reverse proxy
description: Skonfiguruj PiChamber poprawnie za Nginx, Nginx Proxy Manager lub innym reverse proxy.
---

# Reverse proxy

Skorzystaj z tej strony, jeśli uruchamiasz PiChamber za Nginx, Nginx Proxy Manager, Caddy, Cloudflare lub innym reverse proxy.

## Zanim ustawisz proxy

1. Najpierw potwierdź, że PiChamber działa bezpośrednio.
...
- Wyłącz gzip na proxy, jeśli PiChamber już kompresuje odpowiedzi.
...
- PiChamber dostępny bezpośrednio w LAN
...
- PiChamber kompresuje odpowiedzi HTTP za pomocą gzip (próg 1 KB).
...
Trasy strumieniowania SSE są wyłączone z kompresji przez PiChamber, ale CDN może je nadal buforować. Sprawdź dokumentację swojego CDN, jak wyłączyć buforowanie na ścieżkach SSE.
