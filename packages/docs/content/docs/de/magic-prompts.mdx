...
description: Passe die eingebauten Prompts an, die hinter PiChambers automatisierten Abläufen stehen.
---
...
Einige Prompts haben einen sichtbaren Teil (die Nachricht, die du sehen würdest) und einen Anweisungsteil (versteckte Hinweise für den Agenten). Prompts können `{{placeholders}}` enthalten, die PiChamber einsetzt, zum Beispiel den Diff oder den Issue-Titel — lass diese unverändert.
