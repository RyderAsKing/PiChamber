---
title: Tunnel
description: Stelle PiChamber sicher für Remote- und Mobilzugriff bereit.
---

# Tunnel

Ein Tunnel ist ein öffentlicher Link zu deinem PiChamber, damit du ihn von einem normalen Browser in einem anderen Netzwerk aus erreichen kannst. Nutze `pichamber tunnel`, um einen für eine laufende Instanz zu erstellen.
...
PiChamber startet das CLI des Tunnel-Anbieters auf deinem Rechner. Installiere zuerst den Anbieter, den du verwenden möchtest:
...
Wenn du diesen Schritt überspringst, kann `pichamber tunnel start` automatisch einen CLI-Server starten. Beim automatischen Start kannst du Serveroptionen wie `--port`, `--host`, `--lan`, `--ui-password` und `--api-only` mitgeben.
...
Sobald der Tunnel aktiv ist, zeigt `status` eine öffentliche URL. Öffne sie oder scanne den QR-Code, um PiChamber von überall aus zu erreichen.

Standardmäßig druckt PiChamber in interaktiven TTY-Sitzungen einen QR-Code. Mit `--qr` erzwingst du die QR-Ausgabe, mit `--no-qr` deaktivierst du sie.
...
- ein aktiver Tunnel pro PiChamber-Instanz (Port)
...
- [PWA & Mobile](/mobile/) — PiChamber vom Telefon aus erreichen
