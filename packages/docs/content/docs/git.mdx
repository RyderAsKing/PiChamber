...
description: Stage, commit, and manage branches without leaving PiChamber.
---

# Git & GitHub Workflows

PiChamber has a built-in git view so you can review changes, commit, and manage branches without switching to a terminal. Open it from the **Git** tab in the right sidebar.
...
Then write a commit message and commit. You can have PiChamber **generate a commit message** from your staged changes — it uses the current session's model, so you need a session open.
...
If a merge, rebase, or integrate hits a conflict, PiChamber shows what's stuck and lets you resolve it — including handing it to the agent.
