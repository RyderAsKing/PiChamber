...
This package is docs content source-of-truth for PiChamber.
...
- Lead with the task, not background. The first line should say what the page is
  for ("Use `pichamber tunnel` to expose a running PiChamber instance.").
...
- A basic page should fit in a screen or two. Long, dense reference pages (like
  ...
  this page if you run PiChamber behind...").
...
```mdx
3. Run `pichamber --ui-password be-creative-here`.
4. Open the printed URL (usually `http://localhost:3000`).

You should land on the PiChamber session list. If you see it, the server is
running.
```
...
- Explain a term the first time it appears, in parentheses, in everyday words:
  - good: start a tunnel (a public link to your local PiChamber)
...
2. Add frontmatter at top:
   ...
   ---
   title: Remote Access
   description: Access PiChamber from outside your local network.
   ---
   ```
...
The docs are translated into the same languages the PiChamber app ships in.
...
---
title: Встановлення
description: Встановіть PiChamber для десктопа, вебу або VS Code.
---
```
