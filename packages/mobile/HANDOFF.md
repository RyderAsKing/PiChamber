...
- **Deep links** — `pichamber://` URL scheme; a reusable intent vocabulary (`apps/deepLinks.ts`)
...
- `Info.plist`: `CFBundleURLTypes` scheme `pichamber`, `NSCameraUsageDescription`.
- Push entitlement (aps-environment) required.
- APNs `mutable-content: 1` (set server/relay side) wakes the NSE to refresh widgets.

### Android (`android/app`)

- `google-services.json` (committed; Firebase project `pichamber-8bf7e`). The Google Services
