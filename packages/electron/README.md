...
`updater-check.mjs` serializes desktop checks and downloads across windows because Electron keeps one mutable download target. Each new check invalidates the prior pending record before probing; a failed check requires another successful check before download or installation, even if an older installer remains cached. Download completion must match the selected version before it becomes installable. An absent RC release or missing channel manifest permits the other eligible feed to win, but DNS and server failures remain errors rather than authoritative no-update results.
