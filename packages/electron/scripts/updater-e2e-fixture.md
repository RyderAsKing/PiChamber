...
1. Build both versions on the native target architecture. For N and N+1, set the
   ...
   ```bash
   PICHAMBER_TARGET_ARCH=x64 PICHAMBER_UPDATER_E2E_BUILD=1 bun run bundle:main
   PICHAMBER_TARGET_ARCH=x64 node ./scripts/package.mjs --linux --x64 --publish=never
   ```

   Use `PICHAMBER_TARGET_ARCH=arm64` and `--arm64` on an arm64 host. Keep the N and
...
Normal packages omit the build-time marker and always use `RyderAsKing/PiChamber`.
