 ...
 *   PICHAMBER_ELECTRON_PKG_DIR       - resolve the electron package here.
 *   PICHAMBER_ELECTRON_INSTALL_COMMANDS - JSON array of [bin, args] repair
 *                                        commands, e.g.
...
function resolveInstallCommands(env) {
  if (env.PICHAMBER_ELECTRON_INSTALL_COMMANDS) {
    try {
      const parsed = JSON.parse(env.PICHAMBER_ELECTRON_INSTALL_COMMANDS);
      if (
...
export async function main(argv = process.argv.slice(2), env = process.env) {
  ...
  const overrideDir = env.PICHAMBER_ELECTRON_PKG_DIR;
