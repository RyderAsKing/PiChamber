...
const FILE_LINK_SELECTOR = '[data-pichamber-file-link="true"]';
const BLOCK_PATH_TOKEN_ATTR = 'data-pichamber-block-path-token';
const BLOCK_PATH_TOKEN_SELECTOR = `[${BLOCK_PATH_TOKEN_ATTR}]`;
const CODE_BLOCK_PATH_SCANNED_ATTR = 'data-pichamber-block-paths-scanned';
...
const getFileReferenceStatCacheMax = (): number => (
  FILE_REFERENCE_STAT_CACHE_MAX
);

const getFileReferenceLinkLimit = (): number => (
  FILE_REFERENCE_LINK_LIMIT
);
...
// `data-pichamber-block-path-token`. `annotateFileLinks` then promotes those
...
// `data-pichamber-block-paths-scanned` once processed so the walk is not
...
const useFileReferenceInteractions = ({
  ...
  React.useEffect(() => {
    ...
    const annotateFileLinks = () => {
      ...
      for (const candidate of Array.from(candidates)) {
        ...
        void existsPromise.then((exists) => {
          ...
          candidate.setAttribute('data-pichamber-file-path', latestResolved.resolvedPath);
    ...
    const openFileReference = async (sourceElement: HTMLElement) => {
      const raw = sourceElement.getAttribute('data-pichamber-file-ref') || extractPathCandidateFromElement(sourceElement);
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const target = event.target;
      if (!(target instanceof HTMLElement) || target.getAttribute('data-pichamber-file-link') !== 'true') {
        return;
      }
