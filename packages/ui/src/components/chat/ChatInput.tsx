...
} from "./composer/submit/buildOutgoingMessage";
import {
  captureWorktreeAttachments,
  resolveWorktreeSendAttachments,
  worktreeSendAttachmentIds,
} from "./composer/submit/worktreeAttachments";
import {
  isLocalSlashCommand,
  parseSlashCommand,
  tryExecuteLocalSlashCommand,
...
import {
  ...
  ComposerVoiceInput,
} from "./composer/ui/ComposerVoiceInput";
...
const ChatInputComponent: React.FC<ChatInputProps> = ({
  ...
  const pendingStarterInsert = useInputStore((s) => s.pendingStarterInsert);
  const pendingInputText = useInputStore((s) => s.pendingInputText);
  const pendingRevertText = useInputStore((s) => s.pendingRevertText);
  const consumePendingRevertText = useInputStore(
    (s) => s.consumePendingRevertText,
  );
  ...
  const [attachmentPreviewMounted, setAttachmentPreviewMounted] =
  ...
  const isNewSessionStackedComposer = Boolean(
    newSessionDraftOpen && isDesktopStackedComposer && !isComposerExpanded,
  );
  const isInlineComposer =
    !isMobile && !isComposerExpanded && !isDesktopStackedComposer;
  const chatInputRadius =
    isMobile || isDesktopStackedComposer ? "1.5rem" : "9999px";
  ...
  // is the authority; unknown `/tokens` stay plain prose. Bare `/name` is
  // never a skill highlight.
  const { commands: commandCatalogCommands } = useCommandCatalog(currentDirectory);
  const knownSlashNames = React.useMemo(() => {
    const names = new Set<string>();
    for (const command of commandCatalogCommands) names.add(command.invocationName.toLowerCase());
    return names;
  }, [commandCatalogCommands]);

  const availableSnippets = useSnippetsStore((s) => s.snippets);
  const loadSnippets = useSnippetsStore((s) => s.loadSnippets);
  React.useEffect(() => {
    void loadSnippets(currentDirectory);
  }, [currentDirectory, loadSnippets]);
  ...
  React.useEffect(() => {
    const nextKey = chatDraftIdentity
  ...
  const handleQueueMessage = React.useCallback(async () => {
    ...
    if (
      ...
      !currentSessionId ||
      !messageQueueTarget ||
      !areAttachmentsReadyToSend(attachedFiles)
    )
      return;
    ...
    try {
      attachmentsToQueue = await serializeAttachmentsForQueue(
    ...
    if (attachmentsToQueue.length > 0) {
      detachAttachedFiles(
        attachmentsToQueue.map((attachment) => attachment.id),
      );
    }
  ...
  const handleSubmit = async (options?: SubmitOptions) => {
    ...
    const confirmedMentionsAtSend = startsWorktreeTask
      ...
      : null;
    // The pending worktree send's files, captured before the draft switch
    // and the long creation await. Dispatch after setup-ready must use this
    // snapshot, never mutable visible draft state.
    let worktreeAttachmentsAtSend: AttachedFile[] | null = null;
    if (isNewSessionSend) {
      ...
      if (startsWorktreeTask) {
        // Snapshot the pending send's files before the draft switch: upload
        // IDs can expire during setup, so local bytes are retained now for a
        // dispatch-time refresh. Unavailable bytes release this draft's lock
        // and abort before anything is cleared or switched, leaving the
        // draft intact for retry.
        try {
          worktreeAttachmentsAtSend =
            await captureWorktreeAttachments(attachedFiles);
        } catch {
    ...
    try {
        ...
        queuedOnly && claimedFollowUp
          ...
          : {
              ...
              composerText: inputSnapshot.hasContent
                ? inputSnapshot.message
                : null,
              composerAttachments: resolveWorktreeSendAttachments(
                worktreeAttachmentsAtSend,
                attachedFiles,
              ),
              syntheticTexts: syntheticParts?.map((part) => part.text) ?? [],
      ...
      // Conservatively flag any non-local slash input from a new draft.
      // The daemon authoritatively distinguishes registered extension
      // commands from prompts/skills for first-prompt naming; the client
      // must not derive a title from "/balance" before that resolution.
      // Prompt/skill inputs remain safe: skipping the client title lets the
      // daemon supply the same derived title after dispatch.
      const isNewDraftExtensionCommand = Boolean(
        isNewSessionSend
        && parsedCommand
      ...
      if (inputMode !== "shell") {
        try {
          ...
          primaryText = expandText(primaryText, currentSessionDirectory);
          for (const part of additionalParts) {
            if (!part.synthetic)
              part.text = expandText(part.text, currentSessionDirectory);
          }
        } catch (error) {
          console.warn(
      ...
      // Detach only the files this send captured: files added to a newer
      // draft while the worktree built must survive a successful dispatch.
      const composerAttachmentIds = worktreeSendAttachmentIds(
        worktreeAttachmentsAtSend,
        attachedFiles,
      );
      ...
      await sendPromise
        .then(() => {
          ...
          if (!queuedOnly && capturedDraftIsCurrent()) {
            ...
            const sentDraftKey = chatDraftIdentity
              ? getChatDraftIdentityKey(chatDraftIdentity)
              : "";
            const liveDraftIdentity = currentChatDraftIdentityRef.current;
            const liveDraftKey = liveDraftIdentity
              ? getChatDraftIdentityKey(liveDraftIdentity)
              : "";
            const currentInput =
              composerRef.current?.getValue() ?? messageRef.current;
            const composerStillHasSentText =
              !currentInput || currentInput === inputSnapshot.message;
            if (sentDraftKey === liveDraftKey) {
              persistDraftImmediately(
                chatDraftIdentity,
                composerStillHasSentText ? "" : currentInput,
              );
        ...
        .catch((error: unknown) => {
          ...
          if (
            ...
            // Restore the captured mentions: the ref was cleared before the
            // worktree await, so the live ref no longer holds this send's
            // mentions. Matches the worktree-creation failure path.
            if (confirmedMentionsAtSend) {
  ...
  // caret after the trailing space so `$1`/`$@`/args can be typed.
  const insertStarterPrompt = React.useCallback(
    (promptName: string) => {
      const clean = promptName.trim();
      if (!clean) return;
      const prefix = `/${clean} `;
      const editor = composerRef.current;
      const current = editor?.getValue() ?? messageRef.current ?? "";
      // If the composer already holds text, prepend the prompt command so
      // existing draft text becomes trailing arguments; otherwise just the
      // command. One text-and-selection transaction where supported.
      const nextValue = current.trim().length > 0 ? `${prefix}${current.replace(/^\s+/, "")}` : prefix;
  ...
  // insertion via the input store; consume here so focus/caret stay with the
  // mounted composer.
  React.useEffect(() => {
  ...
  const handleStartDictation = React.useCallback(() => {
    ...
    dictationSelectionRef.current = composerRef.current?.getSelection() ?? {
      start: fallback,
      end: fallback,
    };
    closeAutocomplete();
  ...
  const showDraftTargetSelectors =
    (newSessionDraftOpen || Boolean(currentSessionId)) && !isMiniChatSurface;
  ...
  const showComposerTargetRow = Boolean(
    ...
    selectedDraftProject &&
    (newSessionDraftOpen || shouldShowDraftBranchSelector),
  );
  ...
  const pendingChangesBar =
    composerStatusExtrasEnabled && hasPendingChanges ? (
      ...
      >
        ...
        >
          ...
          <MemoStatusRow
            ...
            leftAccessory={
              showComposerTargetRow || newSessionDraftOpen || !pendingChangesBar
                ? null
                : pendingChangesBar
            }
          />
          ...
          (newSessionDraftOpen || shouldShowDraftBranchSelector) ? (
            <div
              ...
              className={cn(
                isComposerLocked && "pointer-events-none opacity-60",
              )}
            >
              <DraftTargetSelectors
                projects={draftProjects}
            ...
            <>
              ...
              >
                ...
                <ComposerFooter
                  ...
                  leadingExtra={
                    isDesktopStackedComposer || isMobile ? (
                      <MemoModelControls
                        keepLabels
                        className="w-max shrink-0"
                        mobilePanel={isMobile ? mobileControlsPanel : undefined}
                        onMobilePanelChange={
                          isMobile ? setMobileControlsPanel : undefined
                        }
                      />
                    ) : null
                  }
                      ...
                      >
                        <ComposerEditor
                          ...
                          onVisualLineCount={(count) => {
                            setComposerVisualLines((previous) =>
          ...
          {isDesktopStackedComposer || isMobile ? null : (
            <div
              className={cn(
                "mt-1.5 flex w-full shrink-0 items-center pl-2",
                isComposerLocked && "pointer-events-none opacity-60",
              )}
              inert={isComposerLocked || undefined}
            >
