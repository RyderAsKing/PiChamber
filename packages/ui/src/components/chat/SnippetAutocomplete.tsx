...
function snippetPreview(snippet: Snippet): string {
  const preview = (snippet.description || snippet.content)
    .replace(/\s+/g, " ")
    .trim();
  return preview.length > 140 ? `${preview.slice(0, 140).trimEnd()}…` : preview;
}

export const SnippetAutocomplete = React.forwardRef<
...
>(({ searchQuery, onSnippetSelect, onClose, style }, ref) => {
  ...
  }, [loadSnippets, effectiveDirectory]);
  ...
  React.useEffect(() => {
    ...
    const matches = query.length
      ? snippets.filter(
          (snippet) =>
            fuzzyMatch(snippet.name, query) ||
            snippet.aliases.some((alias) => fuzzyMatch(alias, query)),
        )
      : snippets;
