...
| `state/` | Composer state with a lifecycle: drafts, mobile shell, history, popup placement, draft targeting, key navigation (`useComposerKeyNavigation`), and autocomplete item selection (`useComposerAutocompleteHandlers`) |
| `submit/` | Turning what the user has into what gets sent |
| `attachments/` | Files: paths, drop payloads (`useComposerDrop`), and clipboard paste handling (`useComposerPaste`). `../../input-store.ts` owns preparation and upload state. |
| `ui/` | Presentation. Dedicated subcomponents isolate visual concerns: `ComposerDragOverlay` renders the drag-over state, the shared Background tasks menu displays asynchronous worktree creation progress and dismissable failure alerts, and `ComposerAttachmentControls` renders the attach button (desktop dropdown, mobile direct picker) immediately to the left of the model picker with the same chrome. Desktop (except mini-chat) uses a stacked card with `1.5rem` corners: editor on top, in-card toolbar with attachments + model picker (name kept visible) + thinking slider after the model name (when the model has levels besides `off`) + send. Dedicated mobile keeps that same stacked composer up at rest (no collapsed pill and no tap-to-grow); the attach button sits immediately to the left of the model and variant controls in the footer with their names always visible and opens the shared native picker (`ATTACHMENT_PICKER_INPUT_PROPS`: `multiple` + `ATTACHMENT_ACCEPT`) directly, reusing the runtime-agnostic preparation/upload pipeline in `input-store.ts`. New-session drafts show folder and local-branch pickers plus a `New worktree` toggle above, with draft starter chips below (prompt chips insert `/name ` and built-in text chips insert literal text, never send immediately). On dedicated mobile, new-session drafts right-align a borderless worktree toggle above the branch, keep the folder at the left of the target row, and omit the changed-file summary. Existing sessions keep their changed-file summary below the read-only branch row. Its tooltip explains that worktrees start from the selected branch's latest commit without copying uncommitted changes. The folder picker includes linked worktrees beneath their owning project. Selecting one targets that directory without registering another project. Selecting a different branch records a draft intent; it never mutates Git from the picker. The first send confirms and completes checkout before creating the session. `New worktree` is an on-demand mode that treats the selected branch as `Start from`, derives a task name with the configured small model (with a local slug fallback), creates a new branch/worktree without changing the source checkout, waits for `setup-ready`, then creates the session and sends against its server-confirmed directory. Existing sessions show only a read-only current-branch label; they never load or display the branch list. Desktop keeps git workspace status on the right of that row (`justify-between`), while dedicated mobile places it below the branch for existing sessions and omits it from new-session drafts. If there is no branch label, git status stays on the row above the composer. Mini-chat keeps a one-line pill with the model picker under the composer. Folder icons in the picker use the sidebar muted grey. Composer thinking is Pi `thinkingLevels`, not a provider-specific `model.variants` field. Opening an existing session locks the composer to that Pi session's last used model and thinking variant so prompt cache identity stays with the transcript; the user can still change them manually. The chrome-less control after the model name opens a compact slider popover above the thinking trigger (centered on it, not flipped below the composer). The card contains the large discrete slider and a min/max legend; the current level stays on the trigger below. The trigger and hover tooltip reserve width for the longest level label so the popover does not drift when the current label changes. Choosing a level writes the composer override and, for an open session, live `sessions.setThinking`. Model picks remain local until send, so a thinking echo for the previous authoritative model must not overwrite the newer manual composer model. Unset/Default does not invent a level. Keyboard `mod+shift+t` and picker `←→` cycle Default plus the model's levels. |
...
- `prefixTokens.ts` — `/` invocations and `#snippet`. Scanning is deliberately
  ...
  skill invocations (`/skill:name`) and Pi-generated extension suffixes
  (`/hello:2`) without weakening path (`a/b` stays prose), snippet, mention,
  or markdown parsing. `#name` is a PiChamber-owned snippet that performs
  literal inline text replacement without Pi prompt variables (`$1`, `$@`
  stay literal); `/name` invokes a Pi prompt template or extension command
  per Pi resolution (extension wins), `/skill:name` invokes a Pi skill (bare
  `/name` never invokes a skill). PiChamber only displays and inserts
...
- `state/useDraftTarget.ts` keeps directory selection separate from branch
  ...
  picker.
- A pending worktree intent is separate from branch-checkout intent and is scoped to runtime, owning project, source directory, and start ref. Each submitted draft receives a unique task identity, so separate drafts can create worktrees from the same source and branch concurrently. The first send captures the draft, derives a safe name, creates the worktree, and polls the server bootstrap phases. Only that submitted draft is read-only while checkout/setup runs; existing sessions and newer drafts remain interactive. The send toasts that the worktree is queued and opens a fresh new-session draft, so the captured prompt materializes into the worktree session on `setup-ready` without stealing the current view. Progress and failures live in the desktop header Background tasks menu and, on dedicated mobile, in the sidebar control above New session. It never materializes the session before `setup-ready` and never falls back to the source checkout. The creation receipt carries the returned path into materialization; the session's returned directory is authoritative afterward. Creating from a linked worktree creates a peer worktree under the same project, and uncommitted source changes are not copied.
...
Send and Queue require every draft attachment to be ready. The handlers repeat that check so keyboard submission cannot bypass it. A new-session send synchronously records the submitting draft ID before its first await, so only that composer locks (send, voice, model, attach, and draft-target controls disable with a pending shimmer) until materialization and the first prompt settle. Existing sessions and newer drafts remain interactive, and runtime switching clears a stale owner. A submitted worktree prompt clears from the generic draft slot as soon as the background task owns it, preventing navigation from restoring it as a new prompt; the snapshot survives receipt while materialization/prompt dispatch is pending, prompt acceptance consumes the exact generation so Background tasks does not linger with a misleading Open action, and post-receipt prompt failure transitions the same entry to failed with the snapshot intact and Restore draft available without relying on the submitted draft still being current (both guarded so a late result never clears a newer generation). Prompt-dispatch-pending entries (`receipt && failedSend && !state`) render as non-destructive `Sending prompt...` with no Open or dismiss/clear actions, and only completed entries announce Worktree ready. Post-receipt empty dispatch settles as failed recovery with the snapshot intact for explicit Restore, while a successfully consumed local command settles as success and detaches only its captured attachments so newer-draft files survive (never a retry that would re-execute it). Pre-receipt creation failure retains the prompt, confirmed mentions, and immutable attachment snapshot in task-owned memory (`failedSend` in `useWorktreeCreationStore`) until an explicit Restore draft action or dismiss, never merging into a newer draft automatically. Re-entering naming/in-flight clears a prior `failedSend` so stale payloads cannot linger. The Background tasks menu offers Restore draft for failed entries: it reopens a new-session draft targeting the original source directory and worktree intent (runtime-guarded; a mismatch, occupied target, or attachment-limit overflow keeps the record with a clear failure and actionable copy), restores confirmed mentions with the prompt, and restores attachments via transactional missing-ID append (all-or-none against the 20-attachment limit; exact limit succeeds) so newer files survive and expired upload ids fall back to the retained data URL. Each Restore draft button keeps the visible copy but carries a task-identifying accessible name (source directory, start ref, and concise task identifier). Different-directory restores open the target draft first and then write the target slot before the identity-switch effect loads it; same-directory restores defer through memory-only `pendingWorktreeRestore` (cleared on runtime reset/switch). Both consume only the same failed payload they read via a guarded store operation, so a newer in-flight/completed/re-failed generation is never cleared. Dismiss and runtime reset release the retained snapshot. A worktree send captures its files before that await (`submit/worktreeAttachments.ts`): the snapshot clones every entry including `uploadState`, strips ephemeral preview object URLs (dispatch previews fall back to the retained `dataUrl`), and retains local bytes so an upload ID that expires during setup can still be refreshed at dispatch. Queueing transfers ownership to the task and immediately detaches only the captured cards without deleting their server uploads. Dispatch after setup-ready uses the snapshot, never live draft state; later cleanup remains scoped to the captured ids, so newer draft files survive. Failure recovery restores only missing captured ids on explicit Restore draft, never overwriting newer draft files. Normal prompt dispatch keeps cards in place until Pi accepts the prompt; failure leaves them available for another attempt. Normal sent text follows the same rule: it stays visible until dispatch settles, then clears on success (only if the composer still shows exactly what was sent and still belongs to that draft) and stays for editing on failure. Sent-message attachments use `MessageFilesDisplay` and do not share the draft-card lifecycle.
...
The package has no DOM test environment, so most coverage stays at the state
and logic layers: the language, the submit assembly, path and drop handling,
text splicing, message history, and the CodeMirror language extension at the
`EditorState` level.
