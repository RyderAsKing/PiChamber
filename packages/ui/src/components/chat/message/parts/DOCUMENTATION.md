...
- The message list folds settled history turns older than the most recent two behind a centered **Load older history** control. That control reveals the two turns immediately above the visible window; **Load all history** restores every folded turn. Neither action changes the session log. Every tool header remains visible regardless of tool count or whether a legacy terminal state has `time.end`; dense turns rely on deferred bodies rather than a **Show N tools** gate. During a stream, token updates patch only the live assistant record when part membership is unchanged; sibling messages and turn activity keep their previous identities so they do not rebuild with the growing text.
