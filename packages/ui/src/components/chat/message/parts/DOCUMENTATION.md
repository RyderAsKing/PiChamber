...
- The message list folds settled history turns older than the most recent two into compact rows. Expanding a folded turn, or jumping to a turn/message inside one, restores that turn's full `ChatMessage` tree without changing the session log.
