...
// xterm.js loads on demand: TerminalView stays eagerly importable for the
// bottom dock without pulling the emulator into the startup graph before a
// terminal is actually mounted.
type XtermModule = typeof import('@xterm/xterm');
type FitModule = typeof import('@xterm/addon-fit');
let xtermRuntimePromise: Promise<{ Xterm: XtermModule; Fit: FitModule }> | null = null;
const loadXterm = () =>
  (xtermRuntimePromise ??= Promise.all([import('@xterm/xterm'), import('@xterm/addon-fit')]).then(
    ([Xterm, Fit]) => ({ Xterm, Fit }),
  ));

// The web entry defers its ~2 MB Nerd Font download until a terminal actually
// mounts (see the `__pichamberEnsureNerdFonts` hook in index.html). Wait for
...
// runtimes without the hook resolve immediately.
const NERD_FONT_WAIT_MS = 2000;
const ensureNerdFonts = (): Promise<void> => {
  if (typeof window === 'undefined') return Promise.resolve();
  const loader = (window as typeof window & { __pichamberEnsureNerdFonts?: () => Promise<void> })
    .__pichamberEnsureNerdFonts;
...
const getProvisionalTerminalSize = (
  ...
  const horizontalPadding =
    (Number.parseInt(style.paddingLeft, 10) || 0) + (Number.parseInt(style.paddingRight, 10) || 0);
  const verticalPadding =
    (Number.parseInt(style.paddingTop, 10) || 0) + (Number.parseInt(style.paddingBottom, 10) || 0);

  // Reserve room for the scrollbar and minimum dimensions, matching the fit addon.
  return {
  ...
  (
    ...
    const fit = React.useCallback(() => {
      ...
      try {
        ...
        if (
          !lastSizeRef.current ||
          lastSizeRef.current.cols !== next.cols ||
          lastSizeRef.current.rows !== next.rows
        ) {
     ...
     * state cleared. `Terminal.reset()` rebuilds VT state while keeping the
     * canvas and font atlas, so prefer it over remounting the whole terminal;
     * the generation bump remains the fallback before the terminal exists.
     */
    ...
    React.useEffect(() => {
      ...
      Promise.all([loadXterm(), ensureNerdFonts()]).then(([{ Xterm, Fit }]) => {
        if (disposed) return;
        const initialTheme = themeRef.current;
        const initialFontFamily = fontFamilyRef.current;
        const initialFontSize = fontSizeRef.current;
        terminal = new Xterm.Terminal({
          ...getXtermTerminalOptions(initialFontFamily, initialFontSize, initialTheme),
        ...
        const attachedTerminal: XtermTerminal = terminal;
        terminal.attachCustomKeyEventHandler((event) => {
          if (event.type !== 'keydown') return true;
          const selection = attachedTerminal.getSelection();
          const outcome = resolveTerminalClipboardShortcut(
            {
              key: event.key,
              code: event.code,
            ...
            clipboardPlatform,
            hasTerminalSelection(selection),
          );
          if (outcome === 'copy') {
            void copyTerminalSelection(selection);
            return false;
        ...
        // OSC 52 clipboard writes stay disabled: remote output must not replace
        // the host clipboard without an explicit user copy gesture.
    ...
    }, [fit, rendererGeneration]);

    React.useEffect(() => {
      const terminal = terminalRef.current;
      if (!terminal) return;
      terminal.options.theme = getXtermTerminalOptions(fontFamily, fontSize, theme).theme;
      terminal.options.fontFamily = getXtermTerminalOptions(fontFamily, fontSize, theme).fontFamily;
    ...
    React.useEffect(() => {
      ...
      terminal.options.cursorBlink =
        isVisible && document.hasFocus() && container.contains(document.activeElement);
    }, [isVisible, ready]);
    ...
    React.useEffect(() => {
      ...
      writeQueueRef.current += pending
        .map((chunk) => (isReplay ? (chunk.replayData ?? chunk.data) : chunk.data))
        .join('');
    ...
    React.useEffect(() => {
      ...
      const cellFromPoint = (clientX: number, clientY: number) => {
        ...
        return getTerminalCellFromPoint(
          clientX,
          clientY,
      ...
      const move = (event: PointerEvent) => {
        ...
        if (gesture === 'selecting') {
          ...
          if (
            focus &&
            (!selectionFocus || focus.column !== selectionFocus.column || focus.row !== selectionFocus.row)
          ) {
