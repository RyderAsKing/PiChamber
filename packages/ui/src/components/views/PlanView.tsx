...
import { parseProjectPlanMarkdown } from '@/lib/pichamberConfig';
...
export const PlanView: React.FC<PlanViewProps> = ({ targetPath = null, onNavigatedToChat }) => {
  ...
  const handleConfirmPlanSend = React.useCallback(
    async (execution: TodoSendExecution) => {
      ...
      try {
        ...
        if (pendingPlanSend.target === 'worktree') {
          ...
          initializeNewPiChamberSession(sessionResult.id, useConfigStore.getState().agents ?? []);
        }
    ...
    [canCreateWorktree, content, createSession, currentProjectRef, initializeNewPiChamberSession, pendingPlanSend, resolvedPath, routeToChat, sendMessage, sendPromptTitle, setCurrentSession]
  );
