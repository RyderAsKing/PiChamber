...
- Layout (web/desktop): top navigation (`SidebarNav`: New session, Multi-run, Archive), then the `recent` zone, then one zone per project with a **flat** session list. There is no rendered worktree grouping level.
...
- Per-project worktree management (`WorktreesView`, opened from the project menu) renders as an overlay inside `<main>` in `MainLayout`; the sidebar no longer mounts it.
...
- `SidebarNav.tsx`: Text navigation rows above the tree (New session, Multi-run, Archive); hidden in VS Code.
