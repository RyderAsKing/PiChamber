...
- **Project zones & session lists**: Project zones display their owned sessions in a unified recency and lifecycle order with per-row branch markers. Sessions created with "Don't work in a folder" have no project owner. They appear only in All folders with a neutral theme-aware row shade and the "No folder" label. Folders render after loose sessions.
