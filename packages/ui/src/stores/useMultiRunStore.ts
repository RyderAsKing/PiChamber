...
import { getWorktreeSetupWaitEnabled, saveWorktreeSetupCommands } from '@/lib/pichamberConfig';
...
const registerCreatedSession = (session: Session, directory: string): Session => {
  ...
  useSessionUIStore.getState().markSessionAsPiChamberCreated(session.id);
