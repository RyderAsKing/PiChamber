...
- `prompt()` prepares its message ID before publishing optimistic busy/pending state. Web, desktop, hosted-mobile, and Capacitor use native `crypto.randomUUID` when available; LAN HTTP browsers use cryptographic `getRandomValues` bytes formatted as UUID v4. Random-source failure rejects without changing session activity. The `msg_` prefix and UUID format remain unchanged.
