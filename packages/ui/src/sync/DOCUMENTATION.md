...
- Reconnect merges the snapshot into the existing cluster without disposals, then hydrates any resident session whose `lastSequence` is behind the resumed cursor so a quiet background turn does not lose the disconnect gap. `getSession` while the daemon is still streaming includes the live assistant/tools and `lifecycle: 'busy'`; a later `session.snapshot` force-hydrates that session so a missed replay window cannot freeze the chat until refresh. Live or sequence-newer resident content overlays a fetch, but a fetched settled transcript whose cursor is at least as new replaces stale overlapping message and part content.
