...
- A draft worktree intent is a runtime, owning-project, source-directory, and start-ref scoped request to create an isolated checkout before materialization. It is distinct from branch checkout, waits for server `setup-ready`, and requires a matching creation receipt whose returned path becomes the requested draft cwd. Failure preserves the draft and never falls back to the source checkout. The submit that starts background creation keeps its captured draft, prompt payload, model configuration, and receipt through completion under a unique draft task ID; selecting another session cannot redirect that prompt. Pending composer state belongs to that draft ID, so existing sessions and newer drafts remain interactive, including when several worktrees are created from the same source and start ref. The submitted prompt is removed from the generic composer draft slot once the task owns it and is retained in task-owned memory (`failedSend`: prompt, confirmed mentions, immutable attachment snapshot) through receipt until prompt dispatch settles, and on pre-receipt failure until an explicit Restore draft action or dismiss. It is never merged into a newer draft automatically. Prompt acceptance consumes the exact generation so Background tasks does not linger with a misleading Open action; post-receipt prompt failure transitions the same receipt entry to failed with the snapshot intact and Restore draft available, without relying on the submitted draft still being current. Both settle paths are guarded to the exact `failedSend` identity, so a late success/failure never clears or overwrites a newer generation. Same-directory restores defer through memory-only `pendingWorktreeRestore` (carrying the exact failed-payload ownership token, cleared on runtime reset/switch); deferred and immediate restores consume only that payload via a guarded store operation, so a newer in-flight/completed/re-failed generation is never cleared. Attachment restores use the transactional missing-ID append (`restoreAttachmentsForRetry`, all-or-none against the 20-attachment limit): exact limit succeeds, overflow leaves visible/stash state untouched, keeps the record, and reports counts for actionable copy. If the captured draft is no longer current, materialization creates and prompts the worktree session without changing the current session or active directory. A draft branch intent is an explicit, runtime-and-directory-scoped request to mutate the draft cwd before materialization. It is held only in runtime memory, clears when project or directory ownership changes, and is never restored from the persisted last-draft target. Materialization rejects a pending intent without a matching checkout receipt. The composer obtains that receipt only after authoritative preflight and confirmed checkout, so cancellation and checkout failure cannot create a session or consume the prompt. The confirmation reads busy/retry lifecycle records from the runtime-wide Pi session catalog on demand and lists active sessions from the selected project and its known worktrees; it rechecks before checkout and requires another click in the same dialog if a new session appeared.
...
- Folder focus sets `focusPending: true` while the list is in flight so the chat can keep its existing view (or its existing PiChamber logo loader) instead of clearing back to `ChatEmptyState`. `sessionsListStatus` distinguishes `'loading'` / `'ready'` / `'failed'` and discriminates authoritative empty success from list failure. A preferred id that is authoritatively missing (`INVALID_SESSION` on the preferred-id `getSession` lookup) commits normal deletion cleanup and falls through to the next active session (or clears when none remains) without retaining the missing id; other lookup failures keep the requested id selected so hydrate can surface a per-session load error
- Same-folder selection is a pointer change on the resident cluster (`select(id)`). Explicit user selection takes precedence over an automatic focus or first-attach list already in flight: the list still commits membership, but a later user-selected id in that directory remains selected through hydration. An initial `open(directory, null)` preserves an earlier selection when its directory is unknown or matches; a different directory cannot inherit that preference. Runtime reset clears the selection intent. Automatic folder focus callers (`MobileApp` connect/directory effect, Electron mini-chat) pass `focusPreferenceForDirectory` (`pi-session-focus-preference.ts`) instead of `null`: the visible chat identity when it belongs to that folder, then a caller-scoped fallback (the mini-chat's configured session), then the store's remembered pick. It returns `null` for the store's current folder (a same-folder focus stays a no-op) and while a draft is active; a session from another folder is never carried in.
...
- `prompt()` prepares its message ID before publishing optimistic busy/pending state via the shared `createBrowserUuid` helper (`@/lib/uuid`). Web, desktop, hosted-mobile, and Capacitor use native `crypto.randomUUID` when available; LAN HTTP browsers use cryptographic `getRandomValues` bytes formatted as UUID v4. Mobile connection, pending-login, and device IDs use the same helper with the same contract. There is no `Math.random` fallback and no global polyfill; random-source failure throws without changing session activity or stored IDs. The `msg_` prefix and UUID format remain unchanged.
...
- Reconnect recovery is replay-driven and separates connectivity from synchronization readiness. `connection: 'ready'` means the transport is alive — a heartbeat is not baseline proof. A contiguous same-epoch replay from the client's own cursor covers every missed event, so nothing is re-fetched and no catalog reload runs. The daemon stamps a snapshot `resync: true` when the requested cursor could not be replayed (window expired or daemon restart); a resync snapshot or a verified `streamEpoch` change queues a bounded recovery pass: affected residents re-hydrate first (selected session outranks the rest, at most two in flight), then every known directory catalog re-lists. `state.syncReadiness` is `'recovering'` while obligations (mirrored in `state.syncRecovery`) are outstanding; a failed scope keeps its retry obligation, retries on a bounded backoff cycle, and is re-queued by later stream-health signals or reconnects — never cleared by them. A newer recovery signal supersedes an in-flight pass: its reads cannot drain the shared obligations, and a fresh pass starts immediately with runtime, epoch, and recovery-revision guards. Partial success commits only the scopes that completed; a failed directory list keeps prior rows and its `'failed'` status. Fetched details commit through `commitHydratedSession`, so live mutations and loaded older history overlay the recovery reads. A verified epoch change additionally resets every resident transcript and per-session cursor (the new daemon's sequence space is unrelated) while preserving optimistic prompts, drafts, attachments, navigation, and catalog metadata, then accepts the lower snapshot baseline of the restarted daemon. Once an epoch-capable daemon establishes its lifetime, unstamped responses and live events are rejected because they cannot be proven to belong to that lifetime. `getSession` while the daemon is still streaming includes the live assistant/tools, `lifecycle: 'busy'`, and extension live state through its returned `lastSequence`; a later `session.snapshot` force-hydrates that session so a missed replay window cannot freeze the chat or lose one-time extension startup state until refresh. Live or sequence-newer resident content overlays a fetch, but a fetched settled transcript whose cursor is at least as new replaces stale overlapping message and part content.

## Ownership map

| Layer / Store | Owns | Scope |
|---|---|---|
| `PiSessionStore` (`apps/pi-session-store.ts`) | Live event stream, reducer `bySession` (LRU-capped transcripts), `hydratedSessionIds`, `lastAccessById`, the live catalog, per-directory refresh generation, runtime-scoped deletion tombstones, and all session mutations (`create` / `rename` / `archive` / `remove` / `fork` / `navigate` / `compact` / `prompt`) | One runtime-wide cluster |
...
| `pi-session-catalog.ts` | Live runtime-scoped metadata catalog (`byId`, `byDirectory`, `listStatusByDirectory`); field-sensitive in-flight list reconciliation with runtime-scoped deletion tombstones; the at-most-2-in-flight directory refresh scheduler | All known directories in the active runtime |
...
| `notification-store.ts` | Turn-complete and error attention plus OS-notification dispatch; fed only by live `PiSessionStore` terminal lifecycle events | Active runtime |
...
| `attachment-snapshots.ts` | Safe attachment cloning for async ownership handoff and recovery: shared immutable bytes, copied mutable upload state, and no revocable preview URL | Worktree send and restore snapshots |
...
Queued auto-send (`useQueuedMessageAutoSend`) dispatches a queued entry only on authoritative live evidence of a terminal lifecycle, never on a transition heuristic. Its tri-state gate (`resolveQueuedAutoSendReadiness`) reads the current runtime's `PiSessionStore` state: `busy`/`retry` hold; `ready` requires `connection === 'ready'`, a catalog row owned by the captured (normalized) directory, and a live-observed terminal (`idle`/`error`) mirror — `record.hydrated`. Everything else is `unknown`, and `unknown` never dispatches: a non-ready connection, a missing row, a colliding session id from another directory, an archived row, and a cold row (the persisted first-paint cache restores every row as `idle` and directory listings carry no lifecycle) cannot invent idle. The dispatcher answers cold rows with an `ensureHydrated` demand — no selection or focus change — whose commit flips `hydrated`, re-emits the catalog, and lets the next scanner pass decide on live state. Demands are deduplicated per target while in flight, verified against authoritative state when they settle (the call resolves even on failure), and retried on the shared bounded backoff instead of stranding the item; that backoff is forgotten once authoritative state arrives or the queue entry is removed, so a later requeue or an evicted-then-revisited session demands again. The demand settles with an explicit scanner wake because a commit can land while the dispatch guard is still held. Missing, colliding, archived, and disconnected targets get no demand at all, so those states hold without looping. An authoritative `INVALID_SESSION` is committed as a normal deletion: the row is removed, its undeliverable queue is cleared with the rest of its persisted session state, no per-session load error is retained, and later passes observe the missing row so no further demand is attempted. The scanner wakes on the existing `catalog` (lifecycle/list edges) and `chrome` (connection) topics only; token deltas mutate neither, so streaming never wakes it, and it reads one catalog row per queued target without projecting transcripts. Dispatch selects via `getAutoSendCandidate` on the latest store — never a stale React snapshot — then claims atomically via the shared `messageQueueStore` claim (`claimQueuedMessage`) before any await — the same semantics manual Send now uses — so a racing manual/auto pair delivers once. The candidate skips persisted confirmed failures (they wait for an explicit Steer and never block unrelated later entries in the same queue) but holds the whole queue behind an earlier transient sending claim or persisted uncertain attempt (intentional FIFO hold — safe against duplication and reorder). It then records the persisted attempt (`kind: 'followUp'` + stable queue id as `operationId` + current `streamEpoch`) synchronously before the send and dispatches `delivery: 'followUp'` with that bound identity, so a turn that went busy between gate and arrival follows up rather than failing `SESSION_BUSY`, a reload holds instead of resending an uncertain delivery, and the exact read-only receipt lookup (`kind + sessionId + operationId`) can reconcile it. Queued send configuration, attachments, runtime key, directory ownership, and current-directory-only scope are re-verified at dispatch time; only the captured id is completed after confirmed acceptance (via `completeQueuedSend`, the only remover allowed while claimed), entries queued during the await stay for their own distinct follow-up, and one failed target never erases unrelated targets. A confirmed rejection clears the attempt and persists a fixed failure label (`sendFailed`, never raw error text): no retry-loop, an explicit Steer may still claim it, and later entries remain auto-sendable. A `PiSendUnconfirmedError`, missing epoch, or retired epoch retains the attempt and holds the queue: no auto-retry and no cross-kind or cross-epoch resend — recovery is the chips' read-only Check status, which removes the entry only on an `accepted` receipt and otherwise keeps it visible with an explicit unknown warning and no replay. The transient `sendingIds` claim drives the Sending indicator and the edit/remove/send guard; `removeFromQueue`/`popToInput` refuse claimed ids on the latest store (not just in UI) so pending uploads cannot be discarded. A captured entry without a variant stays variant-less — never falling back to the mutable current variant.
...
`refreshDirectoryCatalog` is the single fill primitive. It captures a per-directory generation, bumps it on every call, and ignores stale completions (a slow RPC returning after a newer refresh has begun, or after a runtime switch, commits nothing). Every list/focus/open commit also captures a catalog baseline at operation start and reconciles through `applyDirectoryListWithReconciliation` (see Membership rules), so true `title`/`archived` mutations newer than the list start survive without rejecting unrelated complete entities, while lifecycle/hydration-only changes take the listing's authoritative metadata. Deletion tombstones are runtime-scoped, retained across same-runtime cluster rebuilds, and capped at the newest 4,096 ids together with their deletion cursors; runtime switch, clear, and dispose reset them. This bounds long-lived runtimes while covering outstanding bounded RPCs and retained event replay so stale list, detail, or history results cannot resurrect a recent deletion. After a successful directory commit the catalog owner raises frozen session-ordering baselines for that directory's active rows (monotonic, live ranks never demoted). Stale async `archive`/`rename`/`fork`/`clone` completions are rejected by runtime-generation/key guards and mutate nothing on the new runtime. The at-most-2-in-flight scheduler is owned by `pi-session-catalog.ts` (`mapDirectoriesWithRefreshSlot` uses `mapWithConcurrency(2)`; the older nested `withDirectoryRefreshSlot` is exported only for direct callers and must not be re-nested — two limiters can deadlock).
...
`dispose` / `clear` reset the in-memory catalog via `initial()`. `resetForRuntime` flushes the outgoing runtime's pending cache write, clears live state and generations, then seeds the incoming runtime from its own cached snapshot when present. Cached directories remain `'idle'`, so the normal feeder revalidates them. Runtime identities never share catalog rows. The `hydratedSessionIds` set, `lastAccessById`, per-directory refresh generations, and runtime-scoped deletion tombstones all clear in lockstep. Stale async `archive`/`rename`/`fork`/`clone` completions from the previous runtime are rejected before they can mutate the new runtime's catalog.

Ctrl+R reload keeps the active runtime and its last session: the active endpoint (`apiBaseUrl` + `runtimeKey`) is persisted to `pichamber:lastRuntimeEndpoint.v1` and re-hydrated before `getRuntimeKey`/`getRuntimeApiBaseUrl` are used, so a reload does not snap back to `local`. The client token is **not** stored in that cache; Electron restores it from the saved host / local-client-token record before `SessionAuthGate` checks `/auth/session`, and URL-token minting waits for that restore so it cannot race with an injected stale credential. The last active session per runtime is persisted via `last-session-cache` (`oc.lastSession.v1`) on every `setCurrentSession`. When the in-memory `lastSelectedSessionForDirectory` hint is empty after a reload, web/desktop `PiSessionProvider` asks the connected runtime to rediscover that session by id before trusting a directory; a stale client path cannot be sent to another host. Native mobile validates the same pointer through its authoritative catalog restore. `PiSessionCatalogFeeder` ensures the home directory is part of the catalog refresh so a global `~` session is discoverable.
...
`session-activity-timing.ts` measures how long a turn has been running, because `SessionStatus` carries no timestamps. It is driven from `PiSessionStore` event handling (`observeSessionActivityTiming`, `adoptServerRunTiming`, `removeSessionActivityTiming`), so a row can never count a turn that the catalog calls idle. A session gains a start on its first `active` observation and keeps it across repeated busy/retry events; settling converts that start into a finished duration used by live running rows. Unread attention is separate from OS delivery in `notification-store.ts`. Pi lifecycle `idle` after a live active turn appends `turn-complete` for a session that is not currently open; terminal `session.error` or lifecycle `error` appends `error`. Opening the session marks either viewed. User interruption does not notify. OS delivery follows the same live active-to-terminal edge, uses event-stable tags, and is independently gated by the completion/error notification settings. Persisted history and settled bootstrap snapshots never notify.
...
8. Composer and queued sends carry their captured runtime, daemon `streamEpoch`, directory, and session through asynchronous preparation. A runtime or epoch change cancels the send instead of re-resolving it against the new daemon lifetime.
...
Deletion goes through one shared commit (`commitDeletion`): local `remove(sessionId)` (which calls `piClient.deleteSession` with `ignoreMissing` so a `404` still commits locally), an authoritative `INVALID_SESSION` on hydrate or preferred-id lookup, the explicit `session.deleted` event, and missed-deletion baseline cleanup via `commitMissedDeletion`. The commit lands a runtime-scoped tombstone before publishing, then removes the session from the reducer, catalog, `hydratedSessionIds`, ordering, activity timing, revert navigation, navigation/history request state, and any per-session load error, cleans persisted drafts/queues/pins via `cleanupPersistedSessionState`, and re-selects the next active session when the deleted one was current (clears when none remains). An authoritative `INVALID_SESSION` uses this same normal cleanup and navigates away instead of retaining an error page; other load errors (including `SESSION_IN_USE`) keep their per-session error and selection, and list/fetch failures stay non-destructive. A recovery `getSession` that authoritatively returns `INVALID_SESSION` uses the same commit and drains that impossible recovery obligation. Tombstones and retained deletion cursors are capped at the newest 4,096 ids and survive same-runtime cluster rebuilds. Late directory lists, detail hydrates, and history pages that started before the deletion are filtered/rejected against the tombstone; duplicate deletions are idempotent no-ops with the deletion sequence preserved as the per-session cursor; once tombstoned every other live event for that session is skipped wholesale including same-batch events; archive and directory moves keep the id and never enter the tombstone set.

Persisted per-session state (queued messages, folder membership, chat drafts, pins, stashed attachments) is cleaned by `cleanupPersistedSessionState` (`session-deletion-cleanup.ts`), which refuses an identity whose runtime key is no longer active. It is driven by the authoritative baseline cleanup hook (`useAuthoritativeSessionCleanup`): the hook establishes its first authoritative runtime-scoped baseline without deleting anything, and on a later complete authoritative snapshot routes missed deletions through the same shared commit (`commitMissedDeletion`) — never from startup-empty or partially loaded scopes — so a missed deletion also gains tombstone protection against late in-flight responses. Archive and directory moves retain the session ID across snapshots and are not deletion cleanup.
