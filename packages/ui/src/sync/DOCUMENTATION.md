...
1. **Pi runtime-scoped session store** (`PiSessionProvider` / `PiSessionStore`; legacy per-directory stores are not session truth)
...
| `PiSessionStore` (`pi-session-store.ts`) | Live event stream, reducer `bySession` (LRU-capped transcripts), `hydratedSessionIds`, `lastAccessById`, the live catalog, runtime-scoped deletion tombstones, the stream epoch, and per-directory refresh generation | One runtime-wide cluster |
...
Every send is one stable full intent: `operationId`, `messageId` (derived deterministically from the operation id), captured `model`/`thinking`, resolved upload ids plus original attachment fingerprints, delivery kind, owning runtime/session, and the stamped `streamEpoch`. `routeMessage` captures the verified epoch before any upload and remembers the full intent (including `runtimeKey` and fingerprints) before dispatch; transport retries, queue backoff, and manual confirmations of the same operation id reuse every id verbatim — no re-upload, no fresh message id, no fresh epoch — so the daemon fingerprint never mismatches. A cached intent with different text/config/attachments/runtime rejects as `OPERATION_PAYLOAD_MISMATCH` instead of silently reusing old files: ready retries must carry the same resolved ids, same-data retries reuse without re-upload via fingerprints, and a different runtime never reuses another runtime's files. An evicted operation id is never recaptured with a fresh epoch or fresh uploads: the bounded in-memory intent cache (FIFO 256 plus a bounded evicted tombstone) rejects reuse and the caller must use a new operation id. `routeMessage` applies no standalone config writes: the captured model and thinking travel inline on the prompt payload, and the daemon applies them atomically with acceptance inside its per-session config+acceptance lock — so a config failure prevents the Pi call, a duplicate or retry never applies config twice, and concurrent sends from different connections cannot interleave config with another send's acceptance. The daemon deduplicates on the operation id at its execution boundary (claimed before activation, Pi invocation, or any attachment side effect): a retried intent returns the original receipt instead of running a second turn. The queue dispatch derives the id from the queued entry (`queuedSendOperationId`) so its backed-off retries are deduplicated too; composer sends generate one id per submit. A generic `AbortError` after dispatch is uncertain (not proof of rejection): without an explicit caller abort the request may already be running, so it confirms via receipt and stays runtime-scoped. Retried mutations are retried only with this key — the client transport retries reads and explicitly idempotent sends, never a blanket mutation retry. Retention is bounded and explicit: accepted receipts are kept for ten minutes and at most 1,024 entries (oldest evicted to a bounded expired tombstone); pending claims are bounded separately by 256 entries and are never expired while active (expiring an executing claim would permit double execution — a new distinct claim past the bound is rejected as `SESSION_BUSY`). An expired or evicted id is rejected as `OPERATION_EXPIRED` and can never auto-replay: the caller must use a new operation id. The registry is in-memory per daemon process (crash window): a restart loses receipts, the daemon rejects a stale `streamEpoch` pre-side-effect as `STALE_STREAM_EPOCH`, and clients must never replay uncertain sends across a verified epoch change. Queued entries persist explicit fresh-vs-dispatched authority (`operationId`/`messageId`/`streamEpoch`/`runtimeKey`/`sessionId`/`text`/`sendConfig`/`attachmentIds` all-or-nothing plus `dispatched`). Ready manifests are persisted at queue time before any first send; fresh data uploads once through the normal route and persists resolved ids before dispatch, then reuses them verbatim. Rehydration is all-or-nothing and never caches `[]` for data/preparing/failed/expired/unknown manifests (which would silently send text-only). Dispatched entries never fall through to a fresh upload under the same id: a failed rehydration blocks with a requires-new-id error and zero uploads/prompts. Entries without authority (legacy), with a runtime mismatch, with a missing epoch, or with a stale epoch never auto-dispatch after a reload — they are preserved for an explicit new intent (`requeueWithNewIntent`, with a warning) and never auto-mint a fresh id. A missing captured or verified epoch blocks as `missing-epoch`. `OPERATION_PAYLOAD_MISMATCH`, `OPERATION_EXPIRED`, and `STALE_STREAM_EPOCH` surface as actionable outcome-unknown and never poison an endless retry loop; drafts and failed queue entries are preserved. Aborts cancel only the current acceptance (targeted token) and carry a send-generation guard across their own activation await, so an old abort can never kill a newer run. Legacy marker-less SSE subscribers with a cursor are rejected fail-visible (`DAEMON_PROTOCOL_MISMATCH`) instead of silently resuming in the wrong sequence space.
...
- A failed bootstrap is classified as `os-permission` only when the owning runtime filesystem API independently confirms `EPERM`/`EACCES` for that exact directory. Pi or proxy error text is never used as permission evidence. The scheduler retains the directory-scoped reason so local Desktop can offer native folder selection before a forced retry.
...
Every deletion path funnels through one shared commit in `PiSessionStore` (`remove()`, the accepted-`404` hydrate path, the explicit `session.deleted` event, and missed-deletion baseline cleanup via `commitMissedDeletion`). The commit lands a runtime-scoped deletion tombstone before publishing state changes: late directory lists, detail hydrates, and history pages that started before the deletion are filtered or rejected against it, so no in-flight response can resurrect the catalog row, transcript, selection, or activity records. Duplicate deletions are idempotent no-ops, and the deletion event's sequence stays as the per-session cursor so stale replayed events cannot resurrect the session. Once a tombstone exists, every other live event for that session is skipped wholesale — including events that share a `commitEvents` batch with the deletion and events arriving after a local delete whose echo was never observed — so neither the reducer row nor the catalog stub can be resurrected from the event path. Archive and directory moves keep the session id and never enter the tombstone set; the set clears on runtime switch, clear, and dispose, so a same-ID session on a new runtime is a fresh session. The accepted-`404` path commits with `keepSelection` so the failed id stays selected and the chat surfaces its load error while the tombstone still blocks resurrection.

Persisted sidebar state is never reconciled destructively from the first successful startup list. That list establishes an authoritative active+archived baseline. Only a session present in that baseline and omitted from a later complete snapshot is treated as a missed external deletion, and that cleanup routes through the same shared deletion commit (`useAuthoritativeSessionCleanup` → `commitMissedDeletion`) so a missed deletion also gains tombstone protection against late in-flight responses. Archive and directory moves retain the session ID across snapshots and are not deletion cleanup. This favors harmless hidden stale metadata over irreversible user-state loss when startup data is incomplete.
...
The discriminator is whether the server confirmed the path, not whether the value came from local state or a sync response.
...
5. `session-ui-store.ts` delegates message forks without checking the local catalog and without catching errors or showing toasts. The server is authoritative for session existence. Components await the action and own success or failure presentation, so a rejected fork cannot appear successful.
6. Revert navigation retains the original pre-revert leaf across partial restores. A restore that expands only part of the abandoned branch must not replace that leaf with the currently shortened leaf. Once an authoritative navigation restores every abandoned message, clear the temporary revert state. Restore failures reject and preserve the current branch and dock.
7. Composer and queued sends carry their captured runtime, directory, session, model, and thinking through asynchronous preparation. A runtime change cancels the send instead of re-resolving it against the new runtime.
...
9. A prompt send that fails **after** the request left the client is ambiguous, never a definite failure: the server may already be answering it. `classifySendFailure` (`@/lib/pi/send-failure-classification`) reads the transport's ambiguous-failure tag before falling back to typed code/status classification, and distinguishes `rejected` (server responded with an error), `uncertain` (dispatched but outcome unknown: ambiguous tag, `DAEMON_TIMEOUT`, network error, or a generic post-dispatch `AbortError`), and `aborted` (local cancellation via the runtime-switch guard or `SESSION_ABORTED` during acceptance). Only a definite rejection rolls the optimistic send back to a visible failure; an uncertain outcome becomes `confirming` and confirms through the exact authenticated `sessions.sendReceipt` lookup (`kind + sessionId + operationId` plus the stamped epoch and runtime key — never an unrelated lifecycle read, never a resend), and the runtime-generation plus runtime-key guards mean a runtime switch never replays an uncertain intent into the new runtime. Live activity settles turn liveness without confirming acceptance: an authoritative idle can clear optimistic busy while the send remains `confirming`. An `accepted` receipt updates internal acceptance state without a success card; `pending` or a failed receipt lookup starts a bounded read-only retry cycle (five attempts, exponential delays from 200 ms). Exhaustion parks until Check status or a later stream-health/reconnect signal; manual checks cannot race the original HTTP dispatch. Runtime/session/operation/generation guards reject stale completions, and deletion or runtime teardown cancels timers. `expired`/`unknown`, a retired-epoch receipt, or `OPERATION_PAYLOAD_MISMATCH`/`OPERATION_EXPIRED`/`STALE_STREAM_EPOCH` becomes `outcome-unknown`, clearing only an unresolved optimistic turn, not authoritative live activity. The warning preserves the draft; returning to it only dismisses the notice. A new operation id comes from the next explicit composer send or the warned queue action (`requeueWithNewIntent`), never an automatic recovery resend. Concurrent identical sends share one acceptance dispatch; changed payloads under the same id reject, and distinct sends cannot overwrite an in-flight or uncertain owner. Accepted sends release that guard so subsequent steer/follow-up works. `requeueWithNewIntent` never carries a missing (legacy) or stale epoch forward: the owning UI passes the freshly verified current runtime key and stream epoch explicitly, the fresh authority stamps them with a `deriveStableMessageId`-derived message id (the same derivation routing uses — no ad-hoc `msg_` templating), preserves content/config/valid attachments, and is deliberately current-runtime only (an old-runtime target returns null and dispatches nothing). A reloaded pending dispatch reuses that durable queue authority via `rehydrateSendIntentFromQueueEntry` instead of recapturing a fresh epoch or re-uploading under the original uncertain id; expired uploads block instead. Read the send acceptance via `getSendState()` (`sendStateById`: `confirming` / `accepted` / `outcome-unknown` / `rejected`).
...
The Pi server cannot clear `time.archived` over HTTP: `session.update`
...
so `0` reads as active in the UI, the event reducer, and Pi clients.
