...
- A wire disconnect preserves the stream handle and resident cluster, publishes `connection: 'reconnecting'`, and lets the transport retry indefinitely. Hidden/offline backoff wakes immediately on visible/online. An explicit retry or successful native-mobile foreground probe runs `resync()`, which merges the snapshot into the existing cluster without disposals, then hydrates any resident session whose `lastSequence` is behind the resumed cursor so a quiet background turn does not lose the disconnect gap. `getSession` while the daemon is still streaming includes the live assistant/tools and `lifecycle: 'busy'`; a later `session.snapshot` force-hydrates that session so a missed replay window cannot freeze the chat until refresh
