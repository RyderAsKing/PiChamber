...
let syncRenders = 0;
type ButtonCapture = { children: unknown; onClick?: () => void };
...
mock.module('@/components/ui/button', () => ({
  Button: (props: { children?: React.ReactNode; onClick?: () => void }) => {
    buttonCaptures.push({ children: props.children, onClick: props.onClick });
...
mock.module('@/lib/persistence', () => ({ syncDesktopSettings: () => Promise.resolve() }));
...
const { RUNTIME_AUTH_EXPIRED_EVENT } = await import('@/lib/runtime-auth');
...
function installMobileDom() {
  ...
  return {
    ...
    dispatchAuthExpired: () => {
      (win.dispatchEvent as (e: { type: string }) => boolean)({ type: RUNTIME_AUTH_EXPIRED_EVENT });
    },
    restore: () => {
...
function retryButton(): ButtonCapture | undefined {
  for (let i = buttonCaptures.length - 1; i >= 0; i -= 1) {
    const entry = buttonCaptures[i]!;
    if (entry.children === 'Try again' || entry.children === 'Retry now') return entry;
  }
  return undefined;
}

async function clickRetryButton() {
  const entry = retryButton();
  expect(entry?.onClick).toBeDefined();
  await act(async () => {
    entry?.onClick?.();
    await Promise.resolve();
  });
...
describe('MobileApp recovery wiring (mounted)', () => {
  test('queued auto-send stays paused while the transport is uncertain, drains after a verified healthy probe', async () => {
    ...
    expect(shellMounts).toBeGreaterThan(0);
    ...
    // Temporary-unreachable keeps the shell mounted so drafts stay local.
    expect(shellMounts).toBeGreaterThan(0);
    expect(welcomeMounts).toBe(0);
  ...
  test('exhausted recovery restarts on genuine resume/online/manual wakes, never on hidden/offline loops', async () => {
    reprobeHandler = async () => 'unchanged';
    // Healthy cold-start classification first so the app stays mounted.
    const { harness } = await mountMobileApp();
    expect(reprobeCalls).toBe(1);
    reprobeHandler = async () => 'unreachable';

    // One genuine resume enters paced recovery (classification + first cycle probe).
    await resumeViaNative();
    expect(isMobileConnectionUncertain()).toBe(true);
    expect(reprobeCalls).toBeGreaterThanOrEqual(3);

    // Drive the bounded cycle with genuine foreground wakes (retryNow shortens
    // the wait without consuming an extra attempt; 8 unreachable exhaust).
    for (let i = 0; i < 10; i += 1) {
      await resumeViaNative();
      if (retryButton()?.children === 'Try again') break;
      await flush();
    }
    expect(retryButton()?.children).toBe('Try again');
    expect(isMobileConnectionUncertain()).toBe(true);
    const callsAtExhaust = reprobeCalls;
    expect(callsAtExhaust).toBeGreaterThan(0);

    // Offline wake never restarts: no probe burned against a known-dead network.
    harness.setOnline(false);
    await resumeViaNative();
    await flush();
    expect(reprobeCalls).toBe(callsAtExhaust);

    // Hidden wake never restarts either.
    harness.setOnline(true);
    ...
    // Genuine foreground resume restarts a fresh bounded cycle promptly.
    harness.setVisible(true);
    await resumeViaNative();
    await flush();
    expect(reprobeCalls).toBeGreaterThan(callsAtExhaust);
    expect(retryButton()?.children).toBe('Retry now');

    // Genuine online event also restarts from exhausted (manual path below
    // covers the button; online covers the network-change path).
    for (let i = 0; i < 10; i += 1) {
      await resumeViaNative();
      if (retryButton()?.children === 'Try again') break;
    }
    const secondExhaust = reprobeCalls;
    harness.setVisible(true);
    harness.setOnline(true);
    await act(async () => {
    ...
    await flush();
    expect(reprobeCalls).toBeGreaterThan(secondExhaust);

    // Manual retry from the banner wakes the pacing cycle without resetting
    // the endpoint or dropping drafts to the connect screen.
  ...
  test('resume and startup probes share the controller single-owner token (no parallel reprobes)', async () => {
    ...
    // A resume racing the cold-start classification collapses into the
    // in-flight probe instead of opening a parallel transport race.
    await resumeViaNative();
    await flush(2);
    expect(reprobeCalls).toBe(1);
    ...
    // Duplicate resumes while one probe is in flight also collapse to one.
    const second = deferred<'switched' | 'unchanged' | 'unreachable' | 'needs-login' | 'no-connection'>();
    reprobeHandler = () => second.promise;
    const before = reprobeCalls;
    const firstResume = capturedOnResume;
    ...
    await flush(2);
    expect(reprobeCalls).toBe(before + 1);
    second.resolve('unchanged');
    await flush();
    expect(reprobeCalls).toBe(before + 1);
  });
