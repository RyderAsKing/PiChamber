...
export class MobileConnectionRecovery {
  ...
  private startCycle(immediate: boolean): void {
    this.clearTimer();
    this.generation += 1;
    this.attempt = 0;
    this.exhausted = false;
    this.running = true;
    if (immediate) void this.runAttempt(this.generation);
    else this.scheduleNext(this.generation);
  }
  ...
  retryNow(): void {
    ...
    // Manual/online wake shortens the current wait without consuming an extra
    // attempt. runAttempt owns duplicate-probe and offline/hidden handling.
    this.clearTimer();
  ...
  private scheduleNext(generation: number): void {
    ...
    // Delay paces the upcoming probe by its one-based attempt number
    // (1s, 2s, 4s, … capped at 30s). Offline/hidden always uses the long cap
    // and wakes on signals instead of burning attempts.
    const delay = getMobileRecoveryDelay(nextAttempt, {
