...
import { projectSession, type PiProjectedMessage, type PiProjectedSession } from '@/lib/pi/event-reducer';
...
export const PiApp = () => {
  ...
  // Derive from the subscribed snapshot rather than the mutable store instance.
  // The React compiler can otherwise memoize store.selected() against the stable
  // store object and retain the initial null result after hydration.
  const selectedSessionState = state.selectedSessionId
    ? state.reducer.bySession.get(state.selectedSessionId)
    : undefined;
  const selectedSession = selectedSessionState ? projectSession(selectedSessionState) : null;
