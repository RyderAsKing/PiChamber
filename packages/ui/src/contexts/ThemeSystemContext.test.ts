...
const installWindow = (search: string, matchMediaDark: boolean, parentTheme?: unknown) => {
  Object.defineProperty(globalThis, 'window', {
    ...
    value: {
      ...
      parent: parentTheme === undefined ? null : { __pichamberEmbeddedThemeBootstrap: parentTheme },
    },
  });
};
...
describe('ThemeSystemProvider embedded bootstrap', () => {
  ...
  test('publishes the current parent theme without using the URL', () => {
    ...
    expect((globalThis.window as unknown as { __pichamberEmbeddedThemeBootstrap?: unknown })
      .__pichamberEmbeddedThemeBootstrap).toBe(currentTheme);
  });
});
