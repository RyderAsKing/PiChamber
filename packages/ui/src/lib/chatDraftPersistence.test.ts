...
describe('chatDraftPersistence', () => {
  beforeEach(() => {
    storage.removeItem('pichamber.chatDrafts.v2');
  });
  ...
  test('bounds persisted drafts by recency', () => {
    ...
    const envelope = JSON.parse(storage.getItem('pichamber.chatDrafts.v2') ?? '{}') as { drafts?: object };
  ...
  test('reuses a parsed envelope while the stored value is unchanged', () => {
    ...
    storage.setItem('pichamber.chatDrafts.v2', JSON.stringify({
      version: 2,
