...
const readInjectedBearerToken = (): string => {
  ...
  const injected = (window as typeof window & { __PICHAMBER_CLIENT_TOKEN__?: string }).__PICHAMBER_CLIENT_TOKEN__;
...
export const getRuntimeExtraHeadersSync = (): Record<string, string> => {
  ...
  if (typeof window === 'undefined') return {};
  const injected = (window as typeof window & { __PICHAMBER_RUNTIME_HEADERS__?: Record<string, string> }).__PICHAMBER_RUNTIME_HEADERS__;
...
// that could leak token state.
// ---------------------------------------------------------------------------

export const RUNTIME_AUTH_EXPIRED_EVENT = 'pichamber:auth-expired';
