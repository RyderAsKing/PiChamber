...
describe('runtime endpoint switching', () => {
  ...
  test('notifies listeners before and after mutating the active endpoint', () => {
    ...
    try {
      ...
      const sameEndpointGeneration = getRuntimeEndpointGeneration();
      switchRuntimeEndpoint({
        apiBaseUrl: 'https://runtime-b.example',
        runtimeKey: 'runtime-b',
      });
      expect(getRuntimeEndpointGeneration()).toBe(sameEndpointGeneration + 1);
    } finally {
  ...
  test('does not throw when Electron preload globals are read-only', () => {
    ...
    try {
      ...
      Object.defineProperty(runtimeWindow, '__PICHAMBER_API_BASE_URL__', {
        configurable: true,
        value: 'http://127.0.0.1:3000',
        writable: false,
      });
      Object.defineProperty(runtimeWindow, '__PICHAMBER_CLIENT_TOKEN__', {
        configurable: true,
        value: '',
        writable: false,
      });
      Object.defineProperty(runtimeWindow, '__PICHAMBER_RUNTIME_HEADERS__', {
        configurable: true,
        value: {},
        writable: false,
      });
