...
export const switchRuntimeEndpoint = (options: { apiBaseUrl: string; clientToken?: string | null; runtimeKey?: string | null; requestHeaders?: Record<string, string> | null; relay?: RelayRuntimeDescriptor | null }): void => {
  ...
  // Selection generation (mobile retention #10): every explicit endpoint change
  // (including explicit disconnect to `mobile-disconnected` and same-key
  // reconnects) invalidates in-flight background probes. Runtime identity alone
  // cannot catch a disconnect/reconnect flap back to the same key — the
  // generation can.
