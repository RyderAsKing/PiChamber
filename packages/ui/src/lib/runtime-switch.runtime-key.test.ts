...
const setRuntimeWindow = (apiBaseUrl: string | undefined, localOrigin: string | undefined): void => {
  ...
  if (apiBaseUrl !== undefined) runtimeWindow.__PICHAMBER_API_BASE_URL__ = apiBaseUrl;
  if (localOrigin !== undefined) runtimeWindow.__PICHAMBER_LOCAL_ORIGIN__ = localOrigin;
...
describe('getRuntimeKey caching', () => {
  ...
  test('recomputes when the injected API base URL changes at runtime', () => {
    ...
    (globalThis as RuntimeWindow & { window: RuntimeWindow }).window.__PICHAMBER_API_BASE_URL__ = 'https://second.example.com';
  ...
  test('recomputes when the injected local origin changes at runtime', () => {
    ...
    (globalThis as RuntimeWindow & { window: RuntimeWindow }).window.__PICHAMBER_LOCAL_ORIGIN__ = 'https://app.example.com';
    expect(getRuntimeKey()).toBe('local');
  });
});
