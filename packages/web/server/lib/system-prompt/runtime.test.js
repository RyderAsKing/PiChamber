...
describe('managed system prompt runtime', () => {
  it.each(['build', 'plan'])('materializes the optimizer for the %s agent and preserves existing plugins', async (agent) => {
    const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), 'pichamber-system-prompt-'));
    ...
    const pluginPath = path.join(dataDir, 'system-prompt', 'pichamber-system-prompt-plugin.js');
    ...
    const hooks = await pluginModule.PiChamberSystemPromptPlugin();
    const output = {
