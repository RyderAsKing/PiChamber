...
- A client may create before its renderer has mounted. It derives an initial size from the container and font metrics (falling back to 80x24 when unavailable), then sends a resize once the renderer reports its final dimensions. This allows shell startup and renderer initialization to overlap.
- PTY children explicitly clear `NODE_CHANNEL_FD`; daemon IPC descriptors are host-private and invalid after PTY descriptor cleanup. They carry `PICHAMBER_SERVER_TERMINAL=1` so lifecycle commands can distinguish a browser terminal descended from the server from a normal SSH or local shell; startup environment snapshots never persist this marker.
