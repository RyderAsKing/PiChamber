...
import { PiChamberControlError } from '../openchamber-control/error.js';
...
export const createScheduledTaskService = (dependencies) => {
  ...
  const findProjectByID = async (projectID) => {
    ...
    if (!normalized) throw new PiChamberControlError('projectId is required', 400);
    const projects = await listProjects();
    const project = projects.find((entry) => entry.id === normalized) || null;
    if (!project) throw new PiChamberControlError('Project not found', 404);
    return project;
  };
  ...
  const resolveProjectID = async ({ projectId, directory } = {}) => {
    ...
    if (requestedProjectID && requestedDirectory) {
      throw new PiChamberControlError('Provide only one of projectId or directory', 400);
    }
    if (requestedProjectID) {
      await findProjectByID(requestedProjectID);
      return requestedProjectID;
    }
    if (!requestedDirectory) throw new PiChamberControlError('projectId or directory is required', 400);
    ...
    if (!project) throw new PiChamberControlError(`Project not found for directory: ${resolvedDirectory}`, 404);
    return project.id;
  };
  ...
  const findLoopTask = async (projectID, taskID) => {
    ...
    if (!normalizedTaskID) throw new PiChamberControlError('taskId is required', 400);
    const tasks = await scheduledTasksRuntime.syncProject(projectID);
    const task = tasks.find((entry) => entry?.id === normalizedTaskID) || null;
    if (!task) throw new PiChamberControlError('Task not found', 404);
    if (!task.loopFile) throw new PiChamberControlError('Task is not managed by a loop file', 400);
    if (!fs.existsSync(task.loopFile)) throw new PiChamberControlError('Loop file not found', 404);
    return task;
  };

  const setLoopEnabled = async (projectID, taskID, enabled) => {
    if (typeof enabled !== 'boolean') {
      throw new PiChamberControlError('enabled must be a boolean', 400);
    }
    const task = await findLoopTask(projectID, taskID);
    try {
      if (!setLoopFileEnabled(task.loopFile, enabled)) {
        throw new PiChamberControlError('Loop file must be valid before changing its enabled state', 400);
      }
    } catch (error) {
      if (error instanceof PiChamberControlError) throw error;
      const message = error instanceof Error ? error.message : 'Failed to update loop file';
      throw new PiChamberControlError(message, 500);
    }
    const tasks = await scheduledTasksRuntime.syncProject(projectID);
  ...
  const upsert = async (projectID, taskInput) => {
    ...
    if (!taskInput || typeof taskInput !== 'object') {
      throw new PiChamberControlError('task payload is required', 400);
    }
    let upserted;
    try {
      ...
      throw new PiChamberControlError(message, invalid ? 400 : 500);
    }
  ...
  const remove = async (projectID, taskID) => {
    ...
    if (!result.deleted) throw new PiChamberControlError('Task not found', 404);
    await scheduledTasksRuntime.syncProject(projectID);
  ...
  const run = async (projectID, taskID) => {
    ...
    if (result.running || result.queued) {
      throw new PiChamberControlError(result.error || 'Task already running', 409);
    }
    if (result.skipped) throw new PiChamberControlError('Task not found or disabled', 404);
    if (!result.ok) {
      throw new PiChamberControlError(result.error || 'Task run failed', 500, { task: result.task });
    }
