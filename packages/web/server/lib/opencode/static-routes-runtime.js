...
export const createStaticRoutesRuntime = (dependencies) => {
  ...
  const registerApiOnlyFallbackRoutes = (app) => {
    app.get(/^(?!\/api|\/auth|\/health|.*\.(js|css|svg|png|jpg|jpeg|gif|ico|woff|woff2|ttf|eot|map)).*$/, (req, res) => {
      const command = 'pichamber connect-url --help';
...
<head>
  ...
  <title>PiChamber API-only mode</title>
  <style>
    :root {
      color-scheme: dark;
...
<body>
  <main>
    <svg class="logo" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="PiChamber logo">
    ...
    <h1>PiChamber is running in headless mode</h1>
    <p>This server is ready. Open it from the PiChamber desktop or mobile app to use it.</p>
    <div class="command">
  ...
  <script>
    ...
    button?.addEventListener('click', async () => {
      ...
      try {
        ...
        json: () => {
          res.json({ ok: true, mode: 'api-only', message: 'PiChamber is running in API-only mode' });
        },
        default: () => {
          res.type('text/plain').send('PiChamber is running in API-only mode');
        },
      });
    });
  };
