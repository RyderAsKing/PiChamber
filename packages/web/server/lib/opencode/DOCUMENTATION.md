...
- `packages/web/server/lib/opencode/bootstrap-runtime.js`: base app bootstrap runtime for status/auth/tts/notification/PiChamber route wiring.
...
- `packages/web/server/lib/opencode/pichamber-routes.js`: PiChamber update and models metadata route registration.
...
Managed launch env strips AppImage `ARGV0` before spawn so zsh-backed
OpenCode tools do not rewrite child argv[0] to the AppImage path (#2588).
...
- `registerSettingsUtilityRoutes(app, dependencies)`: registers small settings utility endpoints:
  ...
  - `POST /api/config/reload` — applies accumulated deferred OpenCode config changes. Managed OpenCode restarts and returns `requiresReload: true`. External OpenCode returns `requiresManualRestart: true` (changes are already on disk; the connected server must be restarted outside PiChamber).
...
The pipeline binds the PiChamber listener and publishes its active port
before starting managed OpenCode. The managed custom tool therefore receives
an authoritative loopback callback URL even when PiChamber binds port `0`.

## Public exports (pichamber-routes.js)
- `registerPiChamberRoutes(app, dependencies)`: registers PiChamber endpoints:
