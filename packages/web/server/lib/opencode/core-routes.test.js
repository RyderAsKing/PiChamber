...
describe('core-routes', () => {
  ...
  it('redeems pairing sessions with no-store response and generic errors', async () => {
    ...
    expect(response.body).toMatchObject({
      ok: true,
      server: { label: 'PiChamber', url: 'http://runtime.example', fingerprint: 'ABCD-1234' },
...
describe('client auth routes', () => {
  ...
  it('scopes non-desktop client credentials to list and revoke only themselves', async () => {
    ...
    const current = await request(app)
      .post('/api/client-auth/clients')
      .send({ label: 'PiChamber Desktop', clientKind: 'desktop-local' });
  ...
  it('reports null port and tunnel URL on /api/system/info when no getters are wired', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body.pichamberVersion).toBe('1.0.0');
