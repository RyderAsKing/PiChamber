...
export function createSttRuntime({ app, server, express, uiAuthController, isRequestOriginAllowed, rejectWebSocketUpgrade, modelsDir, configFile, liveRevocation = null }) {
  ...
  const trackUpgradePrincipal = (socket, principal, generation) => {
    ...
    try {
      ...
      // A throwing tracker already destroyed the socket above; return an
      // inactive entry so the caller skips handleUpgrade like any other
      // rejected registration instead of upgrading a destroyed socket.
      return { active: false, end: () => {}, revoke: () => false };
    }
