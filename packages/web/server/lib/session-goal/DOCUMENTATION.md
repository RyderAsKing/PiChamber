...
an independent progress auditor. Built on PiChamber's backend-driven
...
`OPENCHAMBER_DATA_DIR` or `~/.config/pichamber`), keyed by the SESSION ID:
...
- `routes.js` — `PUT/GET/DELETE /api/goals/objective/:sessionId`
  (PiChamber-owned, registered before the generic proxy; JSON parsing via
...
1. `createSessionGoalRuntime` subscribes to the global SSE hub and uses the
   envelope's `directory` for its runtime calls.
