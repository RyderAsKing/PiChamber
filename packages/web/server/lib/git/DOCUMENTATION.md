...
- `removeWorktree(directory, input)`: Remove a registered worktree, with optional `force` for dirty files. The service retains internal `deleteLocalBranch`/orphan-cleanup capabilities, but the user-facing route never deletes local branches and rejects unregistered paths.
...
- `DELETE /api/git/worktrees` returns `{ success: true }` after removing a registered non-primary worktree. Its body requires `directory` and accepts `force`; the user-facing operation never deletes the local branch.
...
- Random names use adjectives/nouns from `PICHAMBER_ADJECTIVES` and `PICHAMBER_NOUNS` lists.
- Branches created for new worktrees use `pichamber/<worktree-name>` pattern.
