...
describe('send operation registry', () => {
  ...
  it('accepts a new claim and settles the acceptance receipt', () => {
    const registry = createSendOperationRegistry();
  ...
  it('rejects a payload mismatch for the same id', () => {
    const registry = createSendOperationRegistry();
    const first = registry.claim(baseClaim);
  ...
  it('expires accepted receipts after the ttl so the same id can never auto-replay', () => {
    let time = 1_000;
    const registry = createSendOperationRegistry({ now: () => time, ttlMs: 50 });
    const claim = registry.claim(baseClaim);
    claim.entry.settle({ accepted: true, receipt: { accepted: true, messageId: 'msg-1' } });
    expect(registry.claim(baseClaim).outcome).toBe('accepted');
    ...
    expect(registry.query({ kind: 'prompt', sessionId: 'sess-1', operationId: 'op-1' }).status).toBe('expired');
  ...
  it('bounds retention by evicting the oldest accepted receipts', () => {
    const registry = createSendOperationRegistry({ maxEntries: 2 });
  ...
  it('query() distinguishes pending, accepted, unknown, and expired with no false acceptance', () => {
    let time = 1_000;
    const registry = createSendOperationRegistry({ now: () => time, ttlMs: 50, maxEntries: 10 });
    const key = { kind: 'prompt', sessionId: 'sess-1', operationId: 'op-q' };
    expect(registry.query(key).status).toBe('unknown');

    const fresh = registry.claim({ kind: 'prompt', sessionId: 'sess-1', operationId: 'op-q', fingerprint: baseClaim.fingerprint });
    ...
    const retry = registry.claim({ kind: 'prompt', sessionId: 'sess-1', operationId: 'op-q', fingerprint: baseClaim.fingerprint });
  ...
  it('rejections before Pi leave no tombstone while evicted acceptances expire', () => {
    ...
    expect(registry.query({ kind: 'prompt', sessionId: 'sess-1', operationId: 'op-reject' }).status).toBe('unknown');
    ...
    expect(registry.query({ kind: 'prompt', sessionId: 'sess-1', operationId: 'op-1' }).status).toBe('expired');
    expect(registry.query({ kind: 'prompt', sessionId: 'sess-1', operationId: 'op-2' }).status).toBe('accepted');
  });

  it('never expires an active pending claim so duplicates cannot double-execute', async () => {
    let time = 1_000;
    const registry = createSendOperationRegistry({ now: () => time, pendingTtlMs: 20, maxEntries: 4, maxPending: 2 });
    ...
    expect(registry.query({ kind: 'prompt', sessionId: 'sess-1', operationId: 'op-leak-1' }).status).toBe('pending');
    ...
    expect(registry.query({ kind: 'prompt', sessionId: 'sess-1', operationId: 'op-leak-1' }).status).toBe('unknown');
