...
describe('session daemon stream epoch', () => {
  ...
  it('a legacy client (no epoch marker) never receives a replayed cursor — snapshot safe fallback', async () => {
    const root = await mkdtemp(join(tmpdir(), 'pichamber-pi-daemon-epoch-'));
    await mkdir(join(root, 'agent'), { recursive: true });
    const session = new FakeSession();
    const { endpoint } = await track(root, session);
    ...
    // verify it (no streamEpoch marker). The daemon must not silently trust
    // the raw sequence: the safe fallback is a fresh snapshot baseline.
    const legacy = connectClient(endpoint);
    sockets.push(legacy);
    await legacy.authenticate({ fromSequence: renameEvent.sequence });
    const legacyFrame = await legacy.next((message) => message.kind === 'event');
    expect(legacyFrame.event).toBe('session.snapshot');
    expect(legacyFrame.payload.resync).toBe(true);
    expect(legacyFrame.streamEpoch).toBe(snapshot.streamEpoch);
  });
