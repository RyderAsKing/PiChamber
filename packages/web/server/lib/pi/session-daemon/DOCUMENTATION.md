...
- The private core commands are `runtime.health`, `projects.list/select`, `providers.list`, and session list/create/open/rename/delete/tree/navigate/fork/clone/prompt/steer/followUp/abort/model/thinking/compact operations. `providers.list` projects only catalog and authentication-status metadata, never credentials. `sessions.list` revalidates the configured cwd's JSONL directory before SDK discovery, rejects a different requested directory, and returns no JSONL paths. Session selection validates a persisted JSONL path before Pi opens it; inactive mutations are never directed by a browser-supplied path. Attachment paths originate in the server-local upload map, cross only private IPC, and are redacted before public transcript/event projection. The daemon maps Pi message lifecycle, deltas, tools, queue changes, model/thinking, compaction, and interruption state to canonical events.
