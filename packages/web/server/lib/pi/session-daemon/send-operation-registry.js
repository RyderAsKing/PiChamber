 ...
 *   unrelated work. `streamEpoch` is intentionally NOT fingerprinted: it is
 *   enforced before the claim as a daemon-lifetime guard (`STALE_STREAM_EPOCH`),
 *   so adding or omitting it on a retry in the same lifetime never causes a
 *   mismatch.
 ...
 * the documented crash-window — a client retry across a restart (a verified
 * `streamEpoch` change) may re-execute an accepted intent, and clients must
 * not replay uncertain sends across runtimes. After `ttlMs` the receipt is
 ...
 * duplicate never proceeds on its own. Every claim — including lock-contention
 * (SESSION_BUSY), config, validation, and epoch-mismatch rejections — must
