  ...
  it('streams sequenced projected snapshots without exposing private transport fields', async () => {
    const runtime = {
      ...
      subscribe: async ({ onEvent }) => {
        ...
        onEvent({ protocolVersion: 1, kind: 'event', event: 'session.tool.start', sequence: 9, payload: { sessionId: 'pi-session-5', directory: '/workspace', toolCallId: 'skill-read', partId: 'assistant-1:tool:skill-read', messageId: 'assistant-1', name: 'read', state: 'running', metadata: { pichamber: { skill: { name: 'review' } } }, startedAt: 2_000, serverNow: 3_000, privatePath: '/private/skill/SKILL.md' } });
    ...
    expect(text).toContain('"startedAt":2000,"serverNow":3000');
