# Managed PiChamber Agent Tool

## Purpose

This module exposes PiChamber orchestration to agents as one typed OpenCode
custom tool named `openchamber`. It is injected only when PiChamber launches
...
1. The PiChamber HTTP listener binds and publishes its authoritative port.
...
- External OpenCode selected with `OPENCODE_HOST` or skip-start: not injected,
  because PiChamber does not control that process environment.
