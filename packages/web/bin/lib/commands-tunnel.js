...
async function resolveTargetInstance({
  ...
  if (options.all && requireAll) {
    if (running.length === 0) {
      throw new Error('No running PiChamber instance found. Start one with `pichamber serve`.');
    }
    return running;
  }
      ...
      if (rejectDesktopRuntime) {
        ...
        if (!attachability.attachable) {
          if (attachability.reason === 'desktop') {
            throw new Error(
              `Port ${options.port} is used by PiChamber Desktop app. Tunnel attach requires a CLI instance from \`pichamber serve\`.`
            );
          }
          throw new Error(
            `Port ${options.port} is not an attachable PiChamber tunnel instance. Ensure it is healthy and running PiChamber CLI runtime.`
          );
        }
      }
      return found;
  ...
  if (rejectDesktopRuntime) {
    ...
    if (attachableEntries.length > 1) {
      ...
      throw new Error(`Multiple attachable PiChamber instances found: ${ports}. Use --port <port> or --all.`);
    }
    ...
    if (sawDesktop) {
      throw new Error('Only PiChamber Desktop instance(s) detected. Tunnel attach requires a CLI instance from `pichamber serve`.');
    }

    throw new Error('No attachable PiChamber instance found. Start one with `pichamber serve`.');
  }
  ...
  throw new Error(`Multiple PiChamber instances found: ${ports}. Use --port <port> or --all.`);
}
    ...
    switch (subcommand) {
      ...
      case 'ready': {
        ...
        for (const entry of entries) {
          try {
            const { response, body } = await requestJson(entry.port, `/api/pichamber/tunnel/check?provider=${encodeURIComponent(provider)}`);
            if (!response.ok) {
      ...
      case 'status': {
        ...
        for (const entry of entries) {
          try {
            const { response, body } = await requestJson(entry.port, '/api/pichamber/tunnel/status');
            if (!response.ok) {
          ...
          for (const diagnosticsEntry of diagnosticsEntries) {
            try {
              ...
              const { response, body } = await requestJson(
                ...
                `/api/pichamber/tunnel/doctor?${query.toString()}`,
                doctorFetchOptions,
              );
          ...
          if (isReady) {
            ...
            if (isManagedRemote && (hasPortOrOriginIssue || hasTokenIssue)) {
              troubleshootingHints.push({
                ...
                code: '[PORT_MISMATCH]',
                lines: [
                  'Cloudflare target must match the active PiChamber CLI port.',
      ...
      case 'start': {
        ...
        if (!options.explicitPort && canPrompt(options)) {
          ...
          if (runningInstances.length > 1) {
            ...
            if (safeInstances.length === 0) {
              throw new TunnelCliError(
                'All discovered PiChamber instance ports are browser-unsafe. Start or target a safe port (3000, 5173, 8080, or high ephemeral).',
                EXIT_CODE.USAGE_ERROR,
              );
            }
            ...
            if (attachableSafeInstances.length === 0) {
              throw new TunnelCliError(
                'No attachable PiChamber CLI instances found on safe ports. Start one with `pichamber serve --port 3000`.',
                EXIT_CODE.USAGE_ERROR,
              );
            }

            const selectedPort = await clackSelect({
              message: 'Select PiChamber instance port',
        ...
        if (instance?.autoStarted) {
          ...
          healthProgress?.start(`Waiting for PiChamber on port ${instance.port} to become healthy (up to 60s)...`);
          ...
          const healthy = await waitForServerHealth(instance.port, {
            ...
            onTick({ elapsedMs, complete }) {
              ...
              if (delta > 0) {
                ...
                healthProgress.message(`Waiting for PiChamber health (${progressedSeconds}s / 60s)...`);
              }
          ...
          if (!healthy) {
            healthProgress?.stop('PiChamber is still starting');
            throw new Error(
              `PiChamber on port ${instance.port} is still starting after 60s. Startup time can vary by machine performance. ` +
        ...
        if (selectedProfile && mode === 'managed-remote') {
          ...
          const { response: presetResponse, body: presetBody } = await requestJson(instance.port, '/api/pichamber/tunnel/managed-remote-token', {
            method: 'PUT',
        ...
        let body;
        try {
          ({ response, body } = await requestJson(instance.port, '/api/pichamber/tunnel/start', {
            method: 'POST',
            body: JSON.stringify(payload),
            timeoutMs: 60000,
          }));
        } catch (error) {
          if (error instanceof Error && /\/api\/pichamber\/tunnel\/start/.test(error.message) && /timed out/.test(error.message)) {
      ...
      case 'stop': {
        ...
        for (const entry of entries) {
          ...
          try {
            const { response, body } = await requestJson(entry.port, '/api/pichamber/tunnel/stop', {
              method: 'POST',
            });
            if (!response.ok) {
