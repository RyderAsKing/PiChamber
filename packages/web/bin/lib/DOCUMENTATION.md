...
This directory contains the non-entrypoint implementation for the PiChamber CLI. `packages/web/bin/cli.js` should stay thin: it owns bootstrap, command wiring, top-level dispatch, signal/cancel handling, and compatibility exports. Domain logic belongs in these modules.
...
- `commands-lifecycle.js`
  - Implements `pichamber stop` and `openchamber restart`.
...
- `commands-models.js`
  - Prints PiChamber default, favorite, and recent model settings.
...
- `cli-control.js`
  - Sends one typed action request to the authenticated PiChamber control endpoint and maps HTTP failures to CLI exit behavior.
