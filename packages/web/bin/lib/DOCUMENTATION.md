...
- `server-runtime.js`: shared backend runtime selection. Background `serve` prefers Node.js 22 or newer, then Bun. `dev-server.js` uses the same helper for dev servers.

## Server runtime

Background `serve` picks the backend runtime automatically. It reuses the current executable when the CLI itself runs on supported Node. Otherwise it probes `node` on PATH. This stays correct when the CLI is launched through Bun, where `process.execPath` points at Bun. When PATH has no supported Node, it falls back to the configured Bun binary. When neither runtime is usable, the command fails with a clear error instead of starting a half working server.

Foreground `serve --foreground` stays inline in the current process and never spawns a runtime. Startup services reuse the runtime executable that ran `startup enable` through `process.execPath` (the Node or Bun binary, not the package manager) and run the server foreground inline under the service manager. Electron runs the server in process. Docker images launch via Bun in the image entrypoint, but background `serve` still applies the same automatic selection when a supported Node is installed in the container. Dev `dev:server` and `dev:server:watch` use the same Node first order through `../dev-server.js`, which probes once and spawns once. Package management, builds, and tests stay on Bun.
