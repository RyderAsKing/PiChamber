...
React components use `useRuntimeAPIs()` or `useRuntimeAPI()`. Non-React modules use `getRegisteredRuntimeAPIs()` only when hooks are impossible. Do not introduce direct reads of the legacy runtime global `window.__OPENCHAMBER_RUNTIME_APIS__` in feature code.

## VS Code Route Decisions

| Route type | VS Code behavior |
|---|---|
| PiChamber local route | Handle in the webview and bridge to extension host when needed |
...
Register explicit PiChamber handling before generic proxy fallback. Silent empty fallback is not parity.
