...
| `bun run profile:session` | What a streaming assistant response costs. Creates or reuses a session, dispatches a prompt through the authenticated Pi API used by the page, and records until the session reports idle. Reports the long-task distribution, a timeline-trace breakdown, running animations, and output-normalised metrics. |
