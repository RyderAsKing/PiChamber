...
- **Release asset publishing hardened.** Desktop assets stage before publish and upload through bounded HTTP requests with stalled upload bounds, so a slow asset cannot hold the release job open (`release.yml`).

## [1.0.2] - 2026-09-17

Docker server deployment, connected-server identity, and Termux guidance since 1.0.1.

- **Supported Docker server deployment (#145).** The published image `ghcr.io/ryderasking/pichamber` runs the web server only with `PICHAMBER_UI_PASSWORD` required, `PICHAMBER_HOST=0.0.0.0` in-container, and host loopback publish on `32145` by default (`PICHAMBER_HOST_PORT`, `PICHAMBER_BIND_ADDRESS`, `PICHAMBER_DATA_DIR`, `PICHAMBER_VERSION` overrides, per-project `-p`/`DATA_DIR` isolation). The entrypoint pre-creates mount roots as root then permanently drops to UID `1000:1000` via `setpriv` with no capabilities, keeps the server as PID 1 in foreground `serve`, and tolerates read-only/unwritable SSH mounts with warnings instead of failing. Images ship `init: true`, `no-new-privileges`, `/health` healthchecks, OCI labels, and pinned cloudflared `2026.3.0` multi-arch digest, with `docker.mdx`, install/quickstart/security/environment/cli docs, `CONTRIBUTING.md`, and web README coverage for projects, Pi config copy/bind, symlinked dotfiles, backups, upgrades, network access, and ownership (`Dockerfile`, `docker-compose.yml`, `docker-compose.build.yml`, `docker-entrypoint.sh`, `.env.example`, `docker.mdx`, `install.mdx`, `quickstart.mdx`, `security.mdx`, `environment.mdx`, `cli.mdx`, `connect-devices.mdx`).
- **Container server defaults and origin metadata (#145).** Docker images set `PICHAMBER_DEPLOYMENT_KIND=docker` and seed a fresh-store-only `Workspaces` project at `/home/pichamber/workspaces` via `createDockerInitialLocalSettings`; removing it is respected and never re-seeded. `/health`, `/api/version`, and server status report `deploymentKind`, `serverPlatform`, and `serverDistribution` verbatim so clients adapt without probing, backed by `detectLinuxDistribution` (`ID` plus `ID_LIKE` aliases for Arch/Ubuntu/Fedora/CentOS derivatives, null off-Linux or unknown) with a documented contract (`index.js`, `ui-settings-store.js`, `core-routes.js`, `linux-distribution.js`).
- **Connected server identity in the header (#145).** Remote runtimes show a `This server` menu with the deployment/distro icon, platform label, version, and copyable address, replacing the desktop services menu when connected. Copy uses the shared `copyTextToClipboard` fallback, open state is shared with the header shortcut, and Docker/Windows/macOS/Linux-distro icons are added to the sprite (`ConnectedServerMenu`, `serverPlatformIcon`, `serverPlatformLabel`, `Header`, `DesktopServicesMenu`, `clipboard`, `sprite`, `generate-icon-sprite`).
- **Container updates direct to Compose with release publishing (#145).** In-container `pichamber update` stays disabled; `DOCKER_DEPLOYMENT` capability now returns copyable `docker compose pull` / `docker compose up -d` commands with persisted-volume guidance. The Release workflow publishes `linux/amd64`+`linux/arm64` images on stable and RC tags (dry-run aware) and gates release finalization on its success; the manual Docker image smoke workflow stays parked (`package-manager`, `release.yml`, `docker-image.yml`).
- **Fresh-install draft starters preserved (#145).** The desktop settings migration preserves an absent starter list as the fresh-install sentinel for built-in starters instead of persisting an empty list, while still sanitizing legacy skill/command starters idempotently via `buildDraftStarterMigrationPatch` (`persistence`).
- **Tested Termux server setup (#147).** Install docs cover Android Termux with `pkg install nodejs clang make python` plus `npm install -g @pi-chamber/web --nodedir="$PREFIX"` for the node-gyp Android-headers workaround (`nodejs/node-gyp#3355`), normal `serve`/`status`/`stop`/`restart` use, no `nohup` foreground wrapping, and no systemd boot startup (`install.mdx`).

## [1.0.1] - 2026-09-16

Pi completion alerts, supported-runtime gates, and turn-status fixes since 1.0.0.

- **Pi completion and error alerts (#144).** A server-owned Pi daemon watcher delivers completion and terminal-error notifications across web/PWA, Electron, hosted mobile, and Capacitor mobile via Web Push, APNs/FCM relay, and native callbacks. Notifications fire only for completed work or terminal errors with stable deduplication tags, settle affected messages and tools, ignore bootstrap snapshots and interruptions, and suppress background push while an interactive client is visible unless always-notify is set. Bodies exclude prompt text, model output, tool output, and error details, and push registrations live in a private bounded store (`pi-notification-watcher`, `delivery-runtime`, `routes`, `notification-store`, `pi-session-store`, `useNativePushRegistration`, `push`, `notifications`).
- **Terminal errors show once per turn (#144).** Tool-heavy failed turns no longer render the same terminal error on every streaming assistant message; the final assistant message owns the terminal chat error while earlier continuing segments settle without error duplication (`streamReducers`, `event-reducer`).
- **Retired notification preferences removed (#144).** Subtask/question notification toggles and notification templates are removed from UI state, desktop/server persistence, and settings sync with a v20 UI-store migration, leaving Work completed and Errors event rows (`useUIStore`, `settingsSanitizers`, `settingsStoreSync`, `ui-settings-store`, `NotificationSettings`, `settings/search`).
- **Supported runtimes pinned to Node.js >=22.19.0 and Bun >=1.4.0 (#143).** Unsupported Node/Bun versions now fail fast before server startup or startup-service installation with clear upgrade guidance, while help/status/stop remain available. CI pins Bun 1.4.2 and Node 24, Docker uses Bun 1.4.2, `.nvmrc` tracks Node 24, and Electron keeps its bundled Node in-process. Linux x64 is supported on Node 24 and Bun 1.4.0/1.4.2 with Pi 0.85.1, gated by runtime smoke across Node 22.19/24 and Bun 1.4.0/1.4.2 (`runtime-requirements`, `server-runtime`, `cli-startup`, `pi-runtime-smoke`, `runtime-smoke`).
- **Background and dev servers prefer Node with Bun fallback (#142).** Background `serve` reuses the current supported Node executable, then probes PATH `node`, then falls back to the configured Bun binary instead of starting a half-working server; dev servers share the same helper with single-child lifecycle, and foreground serve stays inline with no spawned runtime (`server-runtime`, `dev-server`, `commands-serve`, `cli`).
- **Continuing assistant turns stay live across tool execution (#141).** `assistant.message.end` with `continuing: true` now keeps `streaming: true` and live `streamingMessages` ownership across the text-segment boundary, so the working footer no longer flashes settled mid-turn or freezes on later `tool.start` mutations; terminal and error ownership semantics are unchanged (`streamReducers`, `continuingMessageLiveStatus`).
- **Updater release notes render as markdown.** Desktop update checks consume the full changelog range and render GitHub release bodies with markdown instead of raw text, with fallback parsing that accepts numbered RC versions (`updater-check`, `main.mjs`).
- **Settings navigation icons refreshed.** Settings nav swaps to clearer icons (equalizer, server, brush, chat, voiceprint, puzzle, terminal-box, history, git-repository, global, PiChamber mark) and prunes unused sprite entries (`metadata`, `sprite`, `generate-icon-sprite`).
...
- **Server update channels with RC subscriptions (#139).** The server now supports `stable` versus `rc` update channels alongside the desktop app channel. RC subscribers compare `latest` and `rc` heads and are offered the newest eligible version; stable subscribers stay stable-only (`package-manager`, `routes`, `ui-settings-store`).
- **Desktop app and server channels are configurable.** Settings → About configures the desktop app channel over local Electron IPC and the server channel against the active runtime while the Local instance is active. Both persist host-local in `runtime-state.json` (`desktopUpdateChannel`, `serverUpdateChannel`, default `stable`), reject unknown values, and are searchable in Settings (`DesktopUpdateChannelSettings`, `Header`, `SettingsView`, `desktopSystemActions`, `persistence`, `settings/search`).
- **Managed systemd updates run in a transient worker.** Startup-enabled servers no longer refuse updates from inside `pichamber.service` on generic `INVOCATION_ID`: detection uses the generated unit marker plus cgroup leaf, and installs launch via `systemd-run` outside the service so the updater survives restarts. Non-secret job state persists under the PiChamber run dir with cross-process locking, worker liveness, and stale-job replacement, with deployment capability reporting for Docker, source checkouts, custom units, and ownership mismatches (`update-job-store`, `package-manager`, `cli-startup`).
- **CLI updates coordinate through jobs and systemd delegation.** `pichamber update` runs shared deployment-capability preflight with stable reason codes, delegates browser-terminal and in-service runs to the transient systemd worker, claims jobs for direct runs to prevent concurrent installs, records `installing`/`verifying`/`restarting` progress, and verifies the newly installed global package path. Generated systemd units are marked and systemd markers are stripped from persisted startup env (`commands-update`, `cli-args`, `package-manager`, `DOCUMENTATION.md`, `cli.mdx`).
- **Update installs return pollable jobs.** `POST /api/pi/update-install` returns 202 with a persisted job id and `GET /api/pi/update-install/:jobId` supports reconnect-safe polling across restarts. Capability is checked server-side before claiming jobs, version context flows into the launcher, and PTY shells carry `PICHAMBER_SERVER_TERMINAL` so custom-unit guidance only applies to server-descended terminals (`routes`, `update-job-store`, `terminal/runtime`).
- **Update dialog follows jobs and shows deployment commands.** Web update networking lives in `web-update.ts` with job-state polling across restarts, immediate worker-failure reporting, and lost-job handling. `UpdateDialog` follows `queued`/`installing`/`verifying`/`restarting`, preserves server-provided manual commands, and renders per-command copy actions with errors before commands (`web-update`, `UpdateDialog`, `server-update-events`, `useUpdateStore`, `Header`).
- **Job lifecycle and desktop checks hardened.** Runtime-scoped job observation, serialized newest-eligible desktop checks, and hardened RC feed handling keep one failed entity from blocking unrelated updates (`update-job-store`, `updater-check`, `main.mjs`).
- **Docs explain app/server channels and systemd flow.** App versus server channel ownership, transient-unit updates for startup-enabled servers, persisted job polling, and required manual actions for containers, source checkouts, custom units, and ownership mismatches (`cli.mdx`, `install.mdx`, `packages/web/README.md`, `packages/electron/README.md`, `terminal/DOCUMENTATION.md`).
...
- **Device revocation now ends live access (#9).** Revoking a paired device immediately closes its authenticated event stream, terminal, and dictation connections, whether they are direct or relayed. Sign-out closes every tracked live connection while rotating the JWT secret. New SSE and WebSocket connections re-check the verified credential at establishment, so a revoked credential cannot win an in-flight token race. Revocations made by another process reach the running server within the bounded 15-second credential poll (`client-auth`, `ui-auth`, `terminal`, `stt`, `server`).
- **Event streams recover safely across server restarts.** Each daemon start has an opaque stream epoch. Clients reject stale-epoch events and cursors, then establish an authoritative baseline instead of replaying the wrong process's history. SSE setup and URL-token minting have deadlines; known authentication failures stop retrying and return to the existing authentication flow, while temporary failures keep local work and retry with bounded backoff (`session-daemon`, `transport`, `bootstrap`, `reconnect`, `runtime-auth`).
- **SSE connections have bounded cleanup and backpressure.** Server event streams install cleanup before subscription completes, stop subscriptions on disconnect or shutdown, and close slow readers once their buffered output reaches the 8 MiB limit or their drain deadline expires. Shutdown now has a finite termination deadline, so a stalled client cannot hold the server open (`event-stream`, `routes`, `server`).
- **Reconnect recovery preserves complete state.** Same-epoch replay no longer reloads resident sessions unnecessarily. A verified resync or epoch change queues bounded recovery for selected sessions and known directory catalogs; failed scopes retain retry obligations until they complete, and successful scopes remain usable. Epoch resets preserve drafts, attachments, optimistic prompts, navigation, and catalog metadata while replacing stale transcript cursors (`pi-session-store`, `sync`, `reconnect`).
- **Session deletion stays authoritative across runtimes.** Runtime-scoped deletion tombstones prevent a deleted session from returning from stale cache, replay, or reconnect data. The current-session deletion path opens the correct fresh draft without erasing unrelated session state (`pi-session-store`, `session catalog`).
- **Terminal and mobile connections tolerate temporary transport loss.** Terminal streams reattach after a runtime transport switch. Native mobile clients retain their saved endpoint, cached sessions, and drafts during an outage, pause sends until recovery, and use bounded foreground retries that do not overwrite an explicit disconnect or host switch (`terminal`, `MobileApp`, `mobile connection recovery`).
- **Desktop tray behavior is configurable.** General settings now let desktop users choose separate minimize and close behavior for the system tray. New installs minimize to the taskbar and hide on close; existing installations retain their previous tray preference until changed (`Electron`, `DesktopNetworkSettings`).

## [0.9.6] - 2026-09-10

Send reliability, revision-safe file saves, and file-search correctness release.

- **Deleting the current session opens a fresh draft (#123).** `deleteSession`/`deleteSessions` now route through `session-ui-store` so deleting the visible session clears the selection and opens a new-session draft instead of leaving a deleted transcript on screen. Deleting another session keeps the current selection, failed deletes stay put, and a session selected mid-delete wins without being stolen (`session-ui-store`, `SessionDialogs`, `session-ui-delete-navigation`).
- **`/api/fs/find` resolves workspaces and filters ignored files safely (#124).** Find uses shared workspace resolution with realpath boundary checks, directory stat, and 1–500 limit clamping, keeps result paths in the requested symlink space, and emits directory candidates for `type=directory`. Git-ignore filtering uses bounded concurrency with a shared in-flight cache and a configurable timeout (`PICHAMBER_GIT_CHECK_IGNORE_TIMEOUT_MS`, 2500 ms default), failing closed on error/timeout instead of returning possibly-ignored files, with regression coverage and contract docs (`fs/routes`, `fs/search`, `fs/DOCUMENTATION.md`).
- **Editor saves are revision-guarded with explicit conflict recovery (#125).** Reads expose opaque `v1:<size>:<mtimeMs>[:<sha256>]` revisions (`x-pichamber-file-revision`, `Cache-Control: no-store`), `GET /api/fs/stat` supports `?knownRevision=` fast-path checks, and `POST /api/fs/write` accepts `{ expectedRevision, overwrite }` with create-only `null`, explicit force, idempotent noop on identical content, per-file serialization, atomic temp+rename, and HTTP 409 `{ reason: 'file-revision-conflict' }` on stale writes. The UI keeps a revision cache, drops stale completions across file/runtime switches, reconciles stat updates, and offers reload/overwrite/compare via `FileSaveConflictDialog`; standalone `DiagramView` recovers without losing edits. CORS exposes revision headers for HMR/desktop/Capacitor, and the exact `~` sentinel resolves to the server home before workspace validation (`fileRevisionCache`, `useFileEditorSave`, `useFileViewerModes`, `useFileStatReconciliation`, `FilesView`, `DiagramView`, `files-errors`, `files`, `fs/routes`, `cors`, `workspace/host`).
- **Send operations deduplicate by stable identity with exact receipts (#126).** Clients label prompt/steer/follow-up sends with a stable `operationId`; the daemon claims `kind+session+id` before Pi activation, deduplicates identical retries to the original receipt (`deduplicated: true`), shares pending outcomes, rejects payload mismatches with 409 and expired retention with 410, and serves exact read-only `POST /api/pi/sessions/:sessionId/send-receipt` lookups (`accepted`/`pending`/`expired`/`unknown`) without invoking Pi (`send-operation-registry`, `session-daemon`, `routes`, `protocol`, `client`).
- **Receipts wait for Pi acceptance and survive preparation races (#126).** The daemon confirms SDK preflight before recording send receipts instead of acknowledging rejected prompts, preserves delivery intent across preparation races, separates pending attachment metadata for steering and follow-ups, and documents bounded dedup retention with acceptance/rejection/pending-duplicate/ordering coverage (`session-daemon`, `prompt-operations`, `prompt-preflight`).
- **Uncertain sends recover without replay (#126).** The UI forwards stable operation/message IDs with captured runtime and directory scope, disables automatic retries after uncertain responses, recovers only from exact accepted receipts, and preserves uploads when acceptance is unknown (`pi-session-store`, `pi/client`, `protocol`, `session-ui-message-routing`, `pi-session-connection`).
- **Follow-up and steering delivery is predictable (#126).** Follow-ups stay independent with atomic send claims, drafts and captured config survive dispatch, uncertain delivery pauses for read-only status checks, and capacity overflow rejects without deleting pending messages. `Queue` is renamed to `Follow-up` with saved behavior/defaults preserved, SDK pending counts surface in the composer, and follow-ups remain device-local (`ChatInput`, `QueuedMessageChips`, `messageQueueStore`, `queuedSendReceipt`, `useQueuedMessageAutoSend`, `composer/DOCUMENTATION.md`).
- **Steered turns keep honest timing (#126).** Working status requires ownership of the authoritative stream, steered turns show `Steering agent` live and `Worked for <duration> · Steered` when settled, and settled turns no longer invent a duration before completion (`TurnBlock`, `TurnItem`, `TurnWorkingHeader`, `assistantWorkingState`, `projectTurnRecords`).
- **Workspace edge cases hardened.** Branch-only history requests use the resolvable-base gate so a missing conventional `main` fallback never reaches the Git log API, and Git history scope follows `branchScopeAvailable` (`GitView`, `apps/DOCUMENTATION.md`).

## [0.9.5] - 2026-09-09

Pi-native cleanup, bounded performance, LAN reliability, and refreshed model catalog release.

- **Pi SDK 0.85.1 refreshes the OpenCode model catalog (#122).** Bundled `opencode.json` drops `deepseek-v4-flash-free` and picks up the upstream catalog fixes. Existing Pi sessions, credentials, settings, and custom models stay in place with no manual migration or cache deletion; custom additions and overrides still apply to the updated catalog. Adds disposable-directory catalog/session regression coverage (`sdk-upgrade.test.js`) and upgrade notes (`session-daemon/DOCUMENTATION.md`, `packages/web/README.md`, `@earendil-works/pi-coding-agent`). If a saved default points at a removed model, pick an available replacement.
- **File tree and daemon work stay bounded (#117).** The file tree builds one per-snapshot `fileTreeStatus` index (`openPaths` set, git status map, folder-badge ancestor counts) so rows answer with O(1) lookups instead of scanning changed files per directory, with gating that skips tree work when the Files surface is not visible (`fileTreeStatus`, `useFilesTree`, `FilesView`). The daemon keeps only a contiguous byte- and count-bounded replay suffix (1024 events / 8 MiB wire bytes, whole-event eviction, oversized events broadcast live but not retained with snapshot fallback) plus bounded idle runtime residency and replay (`session-replay`, `session-daemon`, `prompt-operations`). STT no longer queues work after shutdown (`worker-client`).
- **Session catalog is single-owner with concurrent-mutation reconciliation (#118).** The duplicate `useGlobalSessionsStore`/`globalSessions` list is retired; `pi-session-catalog` is the owner with `session-catalog-access` reads and `pi-session-catalog-feeder` wiring, reconciling catalog lists against concurrent create/archive/remove mutations instead of dropping live entries. Mobile last-session restore and widget snapshots move to focused owners (`mobileLastSessionRestore`, `mobileWidgetSnapshot`, `sessionDirectory`), session auto-cleanup threads the owning directory through archive/remove, and per-session selection preferences consolidate into `selection-store` (retiring `contextStore`, `contextUtils`, `permissionUtils`). (`pi-session-catalog`, `session-catalog-access`, `pi-session-store`, `session-ui-store`, `selection-store`, `useSessionAutoCleanup`, `MobileApp`, `AppEffects`).
- **Chat keeps one activity path with safe send gates (#118).** Assistant turns render through a single `TurnActivityRail`/`AssistantMessageBody` path; the duplicate tool-state owner (`useChatMessageToolsState`) is replaced by `useChatMessagePopupState`, unreachable response-lifecycle branches and `assistantMessageTypes` variants are removed, and render comparison covers the retained path (`renderCompare`, `StaticToolRow`, `chatDefaults.regression`). Queued auto-sends gate on live Pi session state instead of persisted history, response-style instructions inject only on the first user prompt (`responseStyle`), and unused session/viewport fields, retired streaming/directory state (`streaming`, `viewport-store`, `global-session-status`, `session-ordering`, `useSessionPrefetch`, bootstrap demands), and dynamic imports for already-bundled stores are removed. (`ChatMessage`, `AssistantMessageBody`, `TurnActivityRail`, `useAssistantMessageLifecycle`, `useQueuedMessageAutoSend`, `ChatInput`, `composer/DOCUMENTATION.md`, `sync/DOCUMENTATION.md`).
- **Retired Pi-duplicated and dead UI state removed (#118).** The Pi-native generic-agent registry/loading/selection layer (agents, `currentAgentName`, agent model selections, agent-mention parsing) and obsolete sync/bootstrap/permission plumbing (`child-store`, `global-sync-store`, bootstrap demands, directory shims, permission/question hooks) are deleted with consumer updates and owning-docs coverage. Chat/file/window defaults are fixed: obsolete presentation preferences (reasoning traces, collapsible thinking, draft persistence, spellcheck, wide layout, line wrap, file icons, changed-files pills, expanded tools, mermaid/user rendering modes, collapsible/sticky user, prompt navigator, split actions, `directoryShowHidden`, `fileViewerPreview`) and `userModelChoice`/`executionMeta` helpers are retired with `MANUAL-ONLY COLLAPSED` reasoning, always-markdown user text, always-wrap code, SVG mermaid, and edit-default file viewers plus regression coverage; server `ui-settings-store` allowlists drop the same retired keys so saved values cannot restore retired behavior. Gitmoji picker/matcher/preferences and `useGitmojiList` are removed. Window chrome is right-side classic minimize/maximize/close on frameless Windows/Linux (native traffic lights on macOS, none on web/mobile) with the persist v19 migration. (`config`, `session-ui-store`, `session-ui-types`, `persistence`, `appearancePersistence`, `ui-settings-store`, `ModelControls`, `useConfigStore`, `WindowsWindowControls`, `Header`, `TitlebarLeftControls`, `PiChamberVisualSettings`).
- **LAN HTTP works without `crypto.randomUUID` (#121).** New shared `createBrowserUuid` uses `crypto.randomUUID` when available and formats `crypto.getRandomValues` bytes as UUID v4 otherwise (no `Math.random` fallback; missing sources throw), so prompt IDs and mobile connections work on insecure LAN HTTP contexts across web, desktop, hosted-mobile, and Capacitor. (`uuid`, `mobileConnections`, `mobileConnectionStorage`, `pi-session-store`, `pi-session-connection`).

## [0.9.4] - 2026-09-07

Settings-safe idle activation, stable run timers across reload, and responsive Linux updates release.

- **Pi settings apply when sessions go idle (#115).** Provider catalog, project trust, resource, behavior, and prompt-template writes now persist immediately without interrupting active turns. Resident-runtime recreation queues behind a revision counter and flushes at the next safe lifecycle edge, staying pending across transient rebuild failures so a later edge retries; new sessions read the committed file at once. Responses carry a `deferred` flag through the public `/api/pi/*` projections so browsers distinguish saved-but-pending activation from immediate effect. Behavior, provider catalog, project trust, and prompt-template surfaces show saved-when-idle info instead of success or `SESSION_BUSY` errors via shared `isDeferredPiMutation` / `isSessionBusyError` helpers, and prompt-template mutations return deferred/busy distinctly instead of reporting a committed write as failed after a runtime switch (`session-daemon`, `prompt-operations`, `routes`, `protocol`, `client`, `mutation-status`, `usePromptTemplatesStore`, `BehaviorPage`, `useProvidersPageState`, `ProjectTrustDialog`, `PromptTemplatesPage`).
- **Agent and tool timers survive reload (#116).** The session daemon keeps a bounded per-session timing cache (2048 entries, cleared on delete and shutdown), projects `startedAt`/`endedAt` through `getSession` hydration, and samples `serverNow` on tool start/update/end (whitelisted on the public route projection) so a second client or browser reload keeps tool origins. The UI normalizes server-epoch stamps to the client clock via a shared `server-clock` helper (older runtimes without `serverNow` pass through), adopts `runStartedAt`/`serverNow` on bootstrap, reconnect, background refresh, and snapshot (busy or retry with a finite start, with repeated busy events no longer beating the first snapshot), hides the thinking-loader elapsed readout while `startedAt` is null, and prefers the server start over the local pending fallback once present so reload resumes the same count instead of flashing a fresh one (`session-daemon`, `routes`, `protocol`, `types`, `server-clock`, `sessionProjection`, `streamReducers`, `bootstrap`, `reconnect`, `session-activity-timing`, `pi-session-store`, `pi-session-connection`, `AgentThinkingLoader`, `ToolPart`, `WorkingPlaceholder`).
- **Hidden history pages keep loading.** `loadOlderMessages` now returns whether another page remains, and the new `useOlderHistoryDemand` hook keeps demand active across cursors: when a loaded page adds no scrollable height (records hidden inside an existing turn gate), it advances to the next server cursor without requiring a scroll event the browser cannot emit at `scrollTop 0`. `MessageList` uses the hook with viewport-anchored prepends, keeping underfill and near-top behavior unchanged (`pi-session-store`, `useOlderHistoryDemand`, `MessageList`, `sync-context`, `sync/DOCUMENTATION.md`).
- **Linux updates stay responsive (#113).** AppImage validation extracts asynchronously (`spawn` with timeout instead of blocking `spawnSync`), so a slow extraction no longer freezes the Electron window, with a heartbeat regression test. New `linux-package-update` support installs downloaded `.deb`/`.rpm`/`pacman` packages through the resolved native manager (`dpkg`/`apt`, `zypper`/`dnf`/`yum`/`rpm`, `pacman`) via an async elevated runner that unescapes electron-updater installer paths; polkit/sudo authentication no longer blocks the window, `desktop_restart` guards against re-entry, and `window-all-closed`/`before-quit` defer while the Linux installer owns shutdown (`linux-appimage-update`, `linux-package-update`, `main.mjs`, `README.md`).
- **Extension mode status stays live.** Stale hydration no longer erases a newer live `mode` extension status when a fetch lands behind an accepted live event, local live extension fields (`notices`, `errors`, catalog/session-tree revisions, editor) survive hydration regardless of sequence, and changing models always reapplies the selected thinking level since Pi resets thinking to the model default during `setModel` (`pi-session-store-helpers`, `session-ui-message-routing`).

## [0.9.3] - 2026-09-07

Large-session reliability, portable settings, and terminal/desktop polish release.

- **Large sessions open via bounded transcript pages (#112).** Opening a session whose projected transcript exceeds the 16 MiB daemon IPC frame no longer fails with a misleading `MALFORMED_DAEMON_RESPONSE`. `sessions.open`, `navigate`, `fork`, `clone`, and `create` now return metadata plus a byte- and count-bounded tail page; older history loads on demand through a new `sessions.messages` daemon command and `GET /api/pi/sessions/:sessionId/messages` (`before`/`limit` validated). The shared 16 MiB frame limit lives in `ipc-protocol.js` so client and daemon cannot drift, complete newline-delimited frames parse before the incomplete remainder is bounded, and oversized versus malformed frames map to distinct `DAEMON_RESPONSE_TOO_LARGE` / `MALFORMED_DAEMON_RESPONSE` 502 codes. Pages that start inside a tool loop anchor to the owning user message, a single unpageable message fails explicitly, and snapshots read the latest assistant directly instead of projecting everything. The UI carries `hasMoreBefore` / `beforeCursor` through `PiSessionDetailResponse` plus `piClient.getSessionMessages`, deduplicates older-page requests per cursor, merges pages with resident live records winning, preserves loaded history across reconnect tail refreshes, and loads older pages on underfill or near-top scroll with viewport anchoring; navigation replaces the branch and drops stale page state (`session-daemon`, `ipc-protocol`, `ipc-client`, `routes`, `protocol`, `client`, `sessionProjection`, `pi-session-store`, `pi-session-store-helpers`, `MessageList`, `sync-context`).
- **Portable preferences separate from machine state (#110).** `settings.json` now holds only allowlisted portable preferences (`__pichamberSettingsScope: portable-v1`) while host, device, and secret-bearing values live in `runtime-state.json`. `/api/pi/ui-settings` keeps its merged route contract, reading both files and splitting writes by scope; legacy flat files migrate automatically (local state written before the portable replacement, atomic writes, malformed files stay errors, unknown fields dropped by allowlist). Copying the portable file to another host no longer imports absolute project paths or that host's home directory. Electron persists install identity, host connections, local credentials, port, and window state in `runtime-state.json` with a pre-migration fallback to the legacy flat file, and the web CLI reads the desktop local port and client token from runtime state with the same fallback (`ui-settings-store`, `main.mjs`, `cli-paths`, `session-daemon/DOCUMENTATION.md`).
- **Terminal paste no longer duplicates input (#111).** Keyboard paste on Linux/Windows (`Ctrl+Shift+V`, `Shift+Insert`) now relies solely on the native xterm paste event, matching the existing macOS path, instead of firing both the native event and a manual clipboard read. One paste gesture produces one terminal input. Context-menu paste is unchanged (no browser paste event on click, so it keeps the manual clipboard read); copy shortcuts, `Ctrl+C` SIGINT behavior, and OSC 52 policy are unchanged (`TerminalViewport`, `terminalClipboard`).
- **Fresh installs keep reasoning expanded (#109).** The `collapseThinkingByDefault` install default flips from `true` to `false`, so new installs show a one-line reasoning trace unless the user opts into collapse. Existing persisted `true` values are preserved; only the missing-value default changes (`useUIStore`).
- **Linux desktop branding.** Linux packages ship the full hicolor icon set (16–1024px) generated from the PiChamber SVG mark via `bun run icons:brand`, `.deb`/`.rpm` gain a proper synopsis/description, and the desktop entry `Comment` is branded (`Run Pi Coding Agent sessions from the PiChamber desktop app.`). AppImage verification now checks the branded comment plus every hicolor icon path (`package.json`, `after-pack`, `verify-linux-appimage`, `generate-brand-icons`).
...
- **Pi daemon isolated by server profile (#103).** Dev/HMR and packaged servers (plus concurrent instances) no longer share or tear down each other's Pi daemon. Profile-scoped state covers sessions, folders, settings, snippets, model config, archive, client-auth pairing/remote-clients, git identity, STT config, and UI auth/passkeys, guarded by a cross-process lock with explicit `DAEMON_PROFILE_IN_USE` / `DAEMON_LOCK_UNAVAILABLE` / `DAEMON_OWNERSHIP_MISMATCH` errors and lease-aware supervisor restarts (`server-profile`, `cross-process-lock`, `supervisor`, `daemon-process`, `session-daemon`, `session-lease`).
- **Composer locks when a session is open elsewhere (#103).** A session leased by another instance now surfaces `SESSION_IN_USE`: send, voice, model, attach, and draft-target controls lock with a `Session is open elsewhere` placeholder instead of retrying into 409s. The transcript is preserved and the lock clears on the next successful hydrate (`ChatInput`, `protocol`, `pi-session-store-helpers`, `ChatContainer`).
- **Settings persist before the first folder (#106).** Appearance and model-preference autosave start right after desktop-settings sync, even with no folder selected. Local retention settings (`autoDeleteEnabled`, `autoDeleteAfterDays`, `sessionRetentionAction`) migrate from hydrated UI state when the server has no value, and providers/agents load in global scope on first run instead of skipping unknown/empty directories (`App`, `persistence`, `appearanceAutoSave`, `useConfigStore`).
- **Model favorites survive restarts (#104).** Model-preference autosave seeds from the current snapshot, flushes immediately on runtime switch and unmount, and only starts after settings sync; mini-chat and mobile init await settings before autosave so favorites are no longer lost across sessions (`modelPrefsAutoSave`, `persistence`, `App`, `renderElectronMiniChatApp`, `renderMobileApp`).
- **Git clone 404 fixed (#101).** Directory clone now posts to the registered `POST /api/fs/clone` route (was `/api/git/clone`), which supports stored Git identities and returns 409 for an existing destination, with server docs and regression coverage (`useDirectoryCloneAndAdd`, `fs/routes`, `fs/DOCUMENTATION.md`).
- **Mobile live feed tightened.** Phone-width chat uses a smaller semantic type scale, narrower message gutters, and compact activity-rail rows, tool indentation, expanded tool bodies, reasoning blocks, and task summaries. Desktop styling and larger controls are unchanged (`mobile.css`, `ChatMessage`, `TurnActivityRail`, `ReasoningPart`, `StaticToolRow`, `ToolPart`, `ToolExpandedContent`, `TaskToolSummary`, `AssistantTextPart`).
- **CLI machine modes are script-safe.** `--json`/`--quiet` seed before argument parsing so syntax errors still honor the strict JSON contract; `pichamber update` propagates warning/error exit codes in JSON and quiet output; `logs --lines` rejects non-positive-integer values instead of silently using the default; tunnel auto-start and `connect-url` auto-serve run quiet and non-interactive; QR art renders only on TTY so non-TTY `--qr` still emits a capturable pairing link; `--token-stdin` streams stdin with a 64KB cap (`bin/cli`, `cli-args`, `commands-update`, `commands-tunnel`, `commands-connect-url`, `cli-tunnel-profiles`, `package-manager`).
- **Documentation refreshed for public launch.** Reworked contributor setup, release guidance, security reporting, and reverse-proxy requirements; repositioned the README around the Pi-native product and added the Discord community link; updated install/quickstart/security/troubleshooting plus a new CLI reference (`cli.mdx`); refreshed mobile/runtime docs and project skills (`CONTRIBUTING.md`, `README.md`, `SECURITY.md`, `REVERSE_PROXY.md`, `packages/docs`, `packages/web/README.md`, `packages/mobile/README.md`, `packages/electron/README.md`, `.agents/skills`).
- **Maintenance:** removed stale OpenChamber-era planning, migration, research, and reference assets (`PROVIDERS_PLAN.md`, pi workstreams, `pichamber-pimigration`, per-message revert plan, research notes, badge/reference screenshots, mobile `HANDOFF.md`, `oc-dev.config` example), dropped the release-workflow Discord notification step, corrected desktop smoke defaults, and renamed the review workflow to `pr-checks` with a simplified release-build test script.

## [0.9.0] - 2026-09-05

Terminal renderer, guided CLI flows, Git header consolidation, and desktop performance recording release.

- **Terminal moves from ghostty-web to xterm.js.** Swaps the renderer to `@xterm/xterm` with the fit addon, keeps one mounted renderer per tab with directory-wide subscriptions so full-screen programs survive tab switches, adds a focused clipboard shortcut policy and xterm-aligned theme, and removes the `ghostty-web` dependency, type shim, patch, and legacy output helpers (`TerminalViewport`, `TerminalTabPane`, `useTerminalSessionStream`, `useTerminalTabsManager`, `terminalClipboard`, `terminalTheme`).
- **Terminal header collapses to a single row (#94).** Replaces the per-terminal tab strip plus new/restart buttons with a `TerminalTabDropdown` using shared dropdown-trigger chrome, portalled into the `ContextPanel` header left of maximize. Hosts without panel chrome keep a single local dropdown row (`TerminalView`, `TerminalTabDropdown`, `ContextPanel`).
- **Terminal viewport interaction fixes (#94).** xterm.js renders to canvas so the Electron native menu reports Copy/Paste as disabled: the shared context menu is now driven from the xterm selection (Copy/Paste/Select All) with the native menu suppressed on every runtime. The full-page terminal main view gains an opt-in close-view button back to chat, matching `DiagramView` (`terminalClipboard`, `TerminalViewport`, `TerminalView`, `MainLayout`).
- **WebSocket auth tokens bind to connections (#99).** `websocket()` accepts an explicit per-connection `oc_url_token` and `refreshRuntimeUrlAuthToken()` returns the token reserved for that dial, so terminal (`TerminalTransport`), Pi event streams (`transport`), and dictation (`DictationClient`) no longer race on the shared token. `clearRuntimeUrlAuthToken(expectedToken)` only clears when the expected token is still current, so a stale pre-open failure cannot invalidate a newer connection (`runtime-url`, `runtime-auth`, `terminalApi`, `transport`, `dictation-client`).
- **Guided serve and update flows (#98).** Explicit `pichamber serve` with no setup flags opens an interactive access, port, authentication, content, and process-mode wizard with a final review and confirmation; bare `pichamber`, flag-driven runs, non-TTY, `--quiet`, and `--json` stay non-interactive. `pichamber update` reviews old → target versions plus package manager and restart impact with `--yes`/`-y` to skip, verifies the installed version after upgrade, distinguishes previous/installed/latest in JSON and quiet output, and reports partial restart failures (foreground instances and stop failures) instead of silently skipping them. Tunnel `start` ends with a review and confirmation unless profile/mode are fully specified, broad `stop` confirms when more than one instance is affected unless `--force` or an explicit port is given, and `logs --quiet` emits raw lines (`commands-serve`, `commands-update`, `commands-tunnel`, `commands-lifecycle`, `commands-logs`, `cli-args`, `bin/cli`, `DOCUMENTATION.md`).
- **Git and diff headers consolidate into one row (#100).** Replaces `GitHeader` two-row layout plus the embedded `DiffView` toolbar with a single `GitUnifiedHeader` (branch, identity, change scope, sync, repository/diff overflow) and `IdentityDropdown`, portalled into new `ContextPanel` `gitHeaderSlot`/`diffHeaderSlot` following the terminal dropdown pattern. Upstream status folds into the sync tooltip, the dead PR chip wiring is removed, and mobile plus non-panel surfaces keep the inline toolbar with no action lost (`GitUnifiedHeader`, `IdentityDropdown`, `SyncActions`, `GitView`, `DiffView`, `ContextPanel`).
- **Git history and graph gain dedicated sidebar buttons (#97).** `History` and `Graph` move out of the Repository views overflow into their own ghost icon buttons (`h-8 w-8`) in the Git header.
- **Folder-cycle navigation follows the sidebar rail (#95).** `Ctrl+Shift+F` changed the active project and session but left the left folder rail highlighting the previous folder because selection was `SessionSidebar`-local. Selection is lifted into `useSidebarSpaceStore` so folder-cycle shortcuts move the rail exactly like a rail click; row-level session selection still never changes the view filter, with `folderCycleSelection.test.ts` regression coverage (`folderCycle`, `useSidebarSpaceStore`, `SessionSidebar`).
- **Electron process performance recording (#96).** Main process samples Electron CPU/memory metrics, Node heap, and web-contents count every 10 seconds and writes newline-delimited JSON under `<userData>/performance`, resuming after restart until disabled. Records carry aggregate process data only (no session IDs, paths, URLs, credentials, prompts, filenames, or message content). Renderer gets/sets enabled state through local-origin-gated desktop IPC, with a Settings → General → Diagnostics toggle plus search indexing (`process-performance-recorder.mjs`, `main.mjs`, `desktopSystemActions`, `DiagnosticsSection`, `PiChamberVisualSettings`, `README.md`).
