...
- **Stream-epoch cursor safety and transport lifecycle hardening (#1/#5/#6).** Epoch-scoped cursoring is now end-to-end: the UI transport stamps every `/api/pi/events` subscribe with the `events.streamEpoch` capability marker and the `streamEpoch` its replay cursor was established under (routed through `routes.js` → supervisor → `ipc-client.js` → daemon authenticate frame), and the daemon refuses to replay a cursor from a retired epoch even when its own sequence numerically overtook it — resyncing from a snapshot instead — while a marker-less legacy subscriber always receives the snapshot safe fallback, so an old client against a new server can no longer silently miss the head of a new sequence space. The transport adopts a wire epoch transition only after a bounded authoritative health probe confirms it against the live daemon, retires the displaced epoch (frames stamped with it are dropped without downgrade, including snapshots and stamped responses at the store level via `retiredStreamEpochs`), and re-establishes the stream when a foreign epoch cannot be verified; `reconnectPiSession` takes the caller's established `streamEpoch`, reports `epochChanged`, and uses the snapshot baseline verbatim across an epoch change instead of `max(old cursor, snapshot)` (`transport.ts`, `reconnect.ts`, `bootstrap.ts`, `pi-session-store.ts`, `routes.js`, `supervisor.js`, `ipc-client.js`, `session-daemon.js`). Transport lifecycle: SSE connects run under a setup deadline that disposes late attempts, URL-token minting is bounded, pending backoff is woken deduplicated by `online`/visibility/resume, the consecutive-failure count resets only after sustained health, a known 401/403 stops retries and notifies the mounted auth flow through the new `pichamber:auth-expired` runtime-auth event (local work preserved), status-less native EventSource errors are classified by a bounded authoritative probe so a transient 503 never becomes a logout, and all timers/listeners are disposed with the handle (`transport.ts`, `runtime-auth.ts`, `useSessionAuthGateController.ts`). Direct-Capacitor EventSource and relay `runtimeFetch` routing are unchanged.
